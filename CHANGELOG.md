...
- **AI Meal & Nutrition Capture with LiDAR Depth Scaling (`Mahlzeiten-Erfassung`):**
  ...
  - **Voice Dictation (Screen A1):** Hold-to-talk dictation via the platform speech recognizer (`speech_to_text`), with a live sound level, a running transcript and an editable result before anything is sent to the AI — dictation mishears numbers, and a silent "150 g" becoming "115 g" would produce a wrong meal. Recognition is attempted on-device first and only falls back to the system's network recognizer when the device cannot do it locally, in which case the sheet says so. Spoken input works either instead of a photo (via `analyzeText`) or alongside one as a `textHint`, which is the more useful case: cooking method and hidden ingredients are exactly what a photo cannot show.
  ...
  - **One Camera for Photo, Barcode and Depth (iOS):** A single `AVCaptureSession` now drives the live preview (as a platform view), passive barcode detection (`AVCaptureMetadataOutput`) and the still photo with its depth map. There are no capture modes to choose between: the camera watches for barcodes while the user frames a plate, and offers a chip when it finds one. This also runs on iPhones without LiDAR, where it simply returns no depth map — which removes the previous behaviour of opening the system camera on top of a live preview.
  ...
  - **Durable Meal Photo Storage (`MealPhotoStore`):** Captured photos are copied into the application support directory with a 320px preview and referenced by *relative* paths. Both matter: the camera hands over files in a temporary directory the system purges, and the iOS container path changes with every app update, so an absolute path in the database is guaranteed to break. Photos are removed together with the meal entry they belong to.
  - **LiDAR Scale Toggle:** "LiDAR-Maßstab senden" in the AI settings, shown only on devices that can measure. It exists so the same meal can be photographed with and without the measurement — otherwise there is no way to tell whether the scale hint improves the estimate at all.
  - **Organic Morphing Cloud Loading Animation (`AiNeuralCloudOrbWidget` & `MealAnalysisScreen`):** Replaced placeholder `CircularProgressIndicator` with a high-performance 60/120fps hardware-accelerated organic thinking cloud animation inspired by modern AI voice/thinking silhouettes. Features dynamic multi-harmonic lobe undulations, continuous seamless rotation, silky ripple wave cascades, an interactive tap-to-charge liquid dye diffusion gradient (infusing primary green cleanly across the unified cloud silhouette without internal circular seams), full Light and Dark mode theming (white cloud on black in dark mode, black cloud on white in light mode), and a clean red cancel action.
  - **Interactive First-Time AI Meal Capture Tour (`AiMealCaptureTourOverlay` & `AiMealCaptureTourService`):** Step-by-step interactive onboarding tour that automatically introduces first-time users to the multi-photo shutter, live barcode scanner (with focused simulated dummy product preview), gallery import, voice dictation, manual text notes, and smart AI analysis. Replaced prominent circle borders with a clean, subtle frosted-glass backdrop blur with a 10-tier progressive Gaussian blur ramp for ultra-soft transitions. Includes a dedicated info icon in the top app bar to replay the guided introduction anytime.
  - **Main App Walkthrough Tour Modernization (`AppTourOverlay` & Settings Replay):** Upgraded the primary app walkthrough with a 10-tier progressive concentric Gaussian blur ramp (`sigma: 1.8` per tier over 30px) for an ultra-soft feathered falloff without hard edges, pill progress indicators, flush inner tab pill cutouts (keeping the top/bottom bar edges blurred while focusing the tab pill), full-size bar encapsulation in Step 1, and smooth bottom-pinned card transitions without upward jump flickers. Added a dedicated "App-Tour ansehen" action in Settings under Support & About to easily replay the walkthrough anytime without resetting local app data.
...
- **Diary Meal Photo Backdrop Alignment & Smart Dividers:** Centered meal photo thumbnail with bilateral horizontal gradient fading to transparent on both left and right edges. Row dividers in diary meal sections now only appear between adjacent entries when neither row has a photo backdrop, preventing visual clutter around photo thumbnails.
- **Unified Review Suggestions & Meal Detail Design:**
  - Harmonized header layout between `AiMealReviewScreen` and `MealEntryScreen` featuring bold meal title, localized meal type and timestamp, prominent total energy display, and macro pills (Protein, Carbs, Fat).
  - Replaced full-width buttons with a compact, centered `+ Add item manually` secondary action matching the meal detail layout.
  - Positioned meal photos flush below the `GlobalAppBar` with a smooth bottom alpha shader mask fade and gentle header text overlap, creating an immersive hero layout without muddy color banding in both Light and Dark themes.
  - Placed photo carousel indicators and overlays outside the bottom alpha shader mask, elevated (`bottom: 42`) for clean visibility above any text overlap, without background pill containers.
  - Integrated the "Farbskala" depth legend toggle inline on the right side of the timestamp row, saving vertical space and ensuring reliable tap responsiveness.
  - Enabled multi-photo LiDAR depth capture and visualization: all photos captured with LiDAR preserve their respective depth maps, enabling full depth map rendering and swiping across every captured meal image in `MealPhotoWidget` with per-photo "Farbskala" synchronization.
  - Restored the full-screen `MealAnalysisScreen` Living Cloud Orb animation and phase transitions when submitting AI meal corrections and feedback from the review screen.
- **Full-Width Camera Viewfinder & Compact Controls (`AiMealCaptureScreen`):** Live camera viewfinder expanded edge-to-edge across the screen width with rounded corners (24px) and zero side margins, maximized within the vertical viewport with tightly grouped, bottom-pinned controls and Light Mode brand accent styling (`DesignConstants.brandAccentColorLightMode`).
- **Smooth Meal Deletion Flow:** Separated delete confirmation from execution in `MealEntryCard`, allowing `Dismissible` to complete its slide-out animation smoothly before the meal entry is deleted from storage, eliminating screen flicker and list jumping during meal deletions.
- **Meal Photo Overlay Rules Live in One Place:** `buildValidatedOverlayItems` is now the single entry point for deciding what is drawn on a meal photo, and both the review screen and the saved meal detail go through it. It applies the area limits, the per-item cap of four regions, cross-item overlap removal, the whole-dish suppression (a region covering more than 65% of the frame means soup, so nothing is labelled) and the cap of five callouts ranked by energy contribution — not by area, because a lettuce leaf takes up plate but no calories. A duplicated "at least two items" rule inside the overlay widget was removed.
- **Regions Are Drawn for Every Piece of a Food:** One item can sit in several places — two pieces of chicken on opposite sides of the plate. All of its regions are now filled in the item's colour, with the label and leader line anchored to the largest one and a small dot marking the others, so the pieces read as one food rather than several. Previously only the first region of each item was drawn.
- **Meal Regions Survive Saving (`MealCaptureMeta`):** Regions and the measured scale facts are stored as JSON on the meal entry, so a saved meal shows the same overlay the review screen showed without asking the model again. The join is by item name and deliberately loose: a renamed item quietly loses its overlay, which is a normal outcome rather than an error.
- **Distance Guidance Reflects an Actual Measurement:** The capture screen showed a fixed "35 cm · Optimaler Abstand" pill regardless of the scene. It now appears only when the measurement fell outside the range LiDAR is dependable at, and says which way to move. In the normal case there is nothing to say and nothing is shown.
- **Privacy-Preserving Telemetry Extensions:**
  - Expanded opt-in, zero-PII telemetry schema to track usage and reliability across new capabilities including Voice Dictation (`voice_dictation_used`, `voice_dictation_completed` with anonymous duration buckets and AI tidy-up flags), Multi-Modal AI Meal Capture (`ai_meal_scan_requested`, `ai_meal_scan_completed` with modality flags, photo counts, and latency buckets), LiDAR Depth Sensing (`lidar_depth_captured`, `lidar_depth_visualized`), AI Review Corrections (`ai_meal_correction_submitted`, `ai_meal_correction_completed`), and Offline OpenFoodFacts Catalogs (`off_catalog_installed`, `off_catalog_updated`).
  - Added new screen view metrics for `meal_analysis` and `voice_dictation_settings`.
  - Upheld strict zero-PII guarantees: no spoken words, voice recordings, transcripts, user text, food names, or location data are ever captured or transmitted.
...
### Fixed
- **Camera and LiDAR Suspended on Nested Screen Navigation:** Prevented background camera, barcode detection, and LiDAR capture sessions from remaining active when pushing child routes (Meal Analysis, Review, Meal Editor, Settings, or system pickers), eliminating unintended battery drain and the persistent iOS camera indicator in deep screens.
- **Two Camera Sessions Competed for the Same Hardware:** The live preview came from the barcode scanner's `QRView` while the shutter opened a second, separate `AVCaptureSession` for photo and depth. Only one client can own the back camera on iOS, so this could produce a black preview, a failed capture, or both in turn. Preview, barcodes and capture now share one session.
- **Photos Were Captured Before the Camera Had Settled:** `capturePhoto` fired immediately after `startRunning()`, before exposure and autofocus had converged, producing dark and blurred captures. The shutter now waits for focus, exposure and white balance to stabilise (up to 1.2s) before firing.
- **Depth Data Never Reached Dart:** Nested platform-channel maps decode as `Map<Object?, Object?>`, so casting `res['depth']` to `Map<String, dynamic>` threw — and the throw was swallowed by the surrounding catch, leaving depth silently unavailable no matter how well the native side worked. A misaligned `asFloat32List` view on the depth buffer would have thrown the same way and now has a copying fallback.
- **Meal Photos Were Stored by Their Temporary Path:** The saved meal referenced the file the camera or picker had just produced, in a directory the system purges — so the photo disappeared, at the latest after an app update. It is now copied into durable storage first, and no path is stored at all if that copy fails.
- **Image Regions Were Discarded Three Times Before Reaching the Overlay:** Rebuilding items after validation, folding duplicate items together, and the repair pass all dropped the regions the model had returned, so the overlay would have stayed empty even once it was wired up. Regions are now carried through validation, merged when two rows for the same food are consolidated (which means that food sits in two places), and re-attached by name after a repair pass, whose prompt only asks for names and grams.
- **Region Validation Was Never Applied:** `ItemRegionValidator` was fully implemented and unit-tested but not called from anywhere in the app, so every region the model returned was drawn unchecked, including the ones the validator exists to reject.

- **One Ingredient Card for Reviewing and for Saved Meals:** The meal detail screen had its own row layout with a `− 160 g +` stepper that existed nowhere else, showed no energy per ingredient, and could not swap a food. It now uses the same card as the review screen, so the two screens are one thing in two states rather than two things that resemble each other. Swapping, deleting, quick amount changes and per-ingredient energy work identically in both.
- **Diary Rows Share One Shape and One Set of Columns:** Standalone entries were a two-line `ListTile` while meal ingredients were a one-line row with the amount glued to the name — which is why nothing lined up and why a long name could produce `40 g89 kcal`. All diary rows now use `DiaryFoodRow` with fixed amount and energy columns, so every value on the screen sits in the same vertical flight regardless of nesting.
- **Meal Groups Are No Longer a Card Inside a Card:** The meal entry rendered its own filled, rounded surface inside the meal-type card, giving three levels of indentation for two levels of meaning. The meal is now a header row followed by ordinary rows, with a thin branch line as the only grouping cue.
- **Meal Detail Shows All Photos of a Meal:** Extra photo paths are carried in `MealCaptureMeta` (no schema change; regions still refer to the first photo), and the detail screen shows them as the same swipeable carousel the review screen uses. The photo also runs full-bleed under the transparent app bar instead of starting flush against it.
- **A Blocking Screen During Analysis:** Requests previously ran with the capture screen still live underneath, so images could be added or removed while they were already being analysed. `MealAnalysisScreen` — currently a deliberate placeholder for a fuller treatment — blocks the input, names the current stage, shows no invented progress percentage, and can be left, in which case a late result is discarded rather than pushed at the user.
- **Restraint in the Review Screen:** The database match is only named when it differs from the item name (it was printed twice), and the confidence chip only appears below 70%, where it is actually worth a second look.
- **Capture Screen Alignment:** The shutter is centred on the screen rather than being the third of four evenly spread buttons, the text field uses the app's own surface instead of a lime-outlined panel with its own blur, and the thumbnail delete badge is a proper 20px target inside the corner radius. The distance pill previously read a hard-coded "35 cm · Optimaler Abstand" and now only appears when a measurement was actually out of range.
...
- **The Meal Photo Sits Behind the Meal's Title, Not Beside It:** A thumbnail in front of the title pushed it in by its own width, so meals and plain entries no longer began at the same edge. The photo is now a backdrop fading out towards the middle of the row, and every row starts at the same place whether it has a photo or not. Swipe actions were also narrowed to the meal header — wrapping the whole block meant a meal's actions competed with those of its own ingredients.
- **The Expand Chevron No Longer Breaks the Energy Column:** Every diary row reserves the same trailing space, empty on rows that have nothing to put there, so a meal's calorie value lines up with the values above and below it instead of being pushed left by its chevron.
- **Meal Detail Loses Its "Done" Button:** Nothing on that screen is a draft, so a confirm button only added a step. Edits are written when the screen is left.
- **Meal Photos Run Edge to Edge:** On the review screen the photo escapes the list's horizontal padding, and on the detail screen it sits below the app bar with a proper gap rather than butting against it. Both keep the rounded top corners.
- **The Review Screen Rendered Nothing:** Making the photo full-bleed with an `OverflowBox` inside a scrolling list gave it an unbounded height, so it laid out at infinite size and took the whole screen down with it. It is now bounded to the photo's height.
- **Back From the Review Screen Returns Where the User Came From:** It dropped back into the viewfinder, which is never what "back" means after reviewing a meal. It now returns to whatever triggered the capture, saved or not.
- **Barcode Detection Can Be Switched Off:** A toggle sits on the left of the shutter, on by default. On a plated meal the constantly reappearing barcode chip was noise; on a packaged product it is the whole point. Voice and text input moved to the right of the shutter together.
- **Meal Detail No Longer Prints Its Own Title Twice:** The name was in the app bar and again below the photo. The heading is gone; energy leads the line with the macros right-aligned beside it.
- **Deleting a Meal Asks Once:** A swipe showed the generic delete confirmation and then the meal's own question about whether to keep its entries — two differently worded dialogs for one action.
...
- **Leaving the Review Screen Asks Before Discarding (`showGlassBottomMenu`):** Back returns all the way to where the capture started, so a stray back swipe would otherwise throw away a finished analysis with nothing to undo it. The confirmation now uses the native `showGlassBottomMenu` with `AppButton.danger` and `AppButton.secondary` styling rather than a generic platform `AlertDialog`.
- **The New Screens Are Localized:** Everything added for meal capture, analysis, review, meal detail, the delete sheet, the re-analysis comparison and the AI settings was written in German. All of it now goes through `l10n` and is translated into English, German, French, Italian and Japanese — 45 new keys, with proper plurals for ingredient and entry counts rather than glued-together fragments.

### Removed
- **Per-Ingredient Localisation on Meal Photos:** The AI was asked for a bounding box per food, which the app validated and drew on the photo. Tested against real meals the coordinates were not dependable — markers landed on the wrong food often enough that the feature would have misinformed rather than helped, and a marker confidently in the wrong place is worse than none. The prompt no longer asks for regions (which also shortens every response), and the model fields, the validator, the overlay and the debug tab that went with them are gone. `MealPhotoWidget` now simply shows the photo, with a swipeable carousel when a capture produced several.
- **Fabricated Token Cost:** The review screen showed "Cost: ~N tokens" with a currency icon, computed as `1200 + itemsCount * 80`. It was never a measurement of anything. Real usage belongs in the developer inspector.
