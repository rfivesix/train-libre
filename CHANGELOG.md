...
- **Hevy-Style Workout Exercise Drag & Drop Reordering (`LiveWorkoutScreen`, `EditRoutineScreen`, `WorkoutLogDetailScreen`, `EditRoutineExerciseCard`, `WorkoutExerciseLogCard`, `ReorderScrollAnchor`):** Overhauled drag & drop reordering across all workout and routine screens following the industry-standard Hevy anchoring model:
  ...
  - **Dynamic Context-Aware Headroom (`calculateDynamicReorderHeadroom`):** Dynamically computes the exact headroom needed based on the touched card's screen position and its index (`relativeY - index * collapsedHeight`), providing just enough top gap so the touched card stays pinned under the finger without overshooting into an 800px black void.
...
- **Phantom ~1000ms App Resume in the Performance Log (`StartupTrace`):** The Performance Log reported a resume of almost exactly 1000ms every single time, and no amount of optimization moved it — because the number was the measuring instrument, not the app. The resume path itself does no blocking work: `MainScreen` only restarts its widget refresh timer, `JankRecorder` only re-bases its watchdog clock, and the Live Activity command drain is already off the frame path.
  - **The Number Was the Engine's Reporting Delay:** A run was closed by reading the clock inside the frame timings callback. The engine batches frame timings and flushes them only once per second in release builds (100ms in debug/profile), and a resume produces exactly one frame before the app goes idle, so nothing else ever filled the batch. Every resume therefore measured the full flush delay and reported ~1000ms ±20ms — a hard floor that no app-side optimization could ever cross. The earlier halving from ~2000ms to ~1000ms removed roughly a second of genuine work and then landed flat on this floor, which is why further tuning appeared to do nothing.
  - **Measured to the Frame, Not to the Callback:** Runs are now closed with the engine's own wall-clock stamp for the frame (`FramePhase.rasterFinishWallTime`) instead of the clock at delivery time, so the reported span ends when the frame actually reached the screen. The batch is scanned for the first frame belonging to the open run; a batch holding only older frames no longer closes it early.
  - **Guarded Against a Moving Wall Clock:** A frame stamped before its run opened or after the callback that reported it means the wall clock jumped (NTP, a manual time change) or the platform left the stamp empty, so the previously measured span is used instead. Phase durations continue to come from the monotonic clock and are unaffected.
  - **Transient Interruptions No Longer Counted:** A Control Centre pull or a permission sheet stops at `inactive`, but any non-`resumed` state was treated as having been in the background, so those opened resume runs and buried the real returns in noise. Only `hidden` and below now count as away.
  - **Interrupted Resumes Discarded:** A resume whose frame never landed before the app was backgrounded again was left open and closed by the first frame of the *next* return, reporting the entire time spent in the background as resume latency.
...
- **Dynamic AI Model Discovery & Error Surfacing (`AiService`, `AiSettingsScreen`):** Fixed issues where the model dropdown in AI settings fell back to hardcoded models without communicating why:
  ...
  - **Inclusive Family Filter & Future-Proof Ranking:** Broadened the OpenAI model filter to allow the `o`-series and unpredicted naming schemes (dropping only non-chat modalities like embeddings, audio, and image generation), while sorting models dynamically by parsed version and capability tiers instead of stale hardcoded bonuses.
  - **Reasoning & Luna Temperature Compatibility:** Added automated temperature compatibility for OpenAI reasoning models (`o1`, `o3`, `o4`) and modern specialized models (`gpt-5.6-luna`, `gpt-5.6-sol`, `gpt-5.6-terra`) which reject custom `temperature` parameters, including automatic fallback retry on HTTP 400 errors.
...
- **iCloud Restore Table Identifier Validation (`ICloudSyncService`):** Strictly validate table names from the untrusted backup snapshot using an alphanumeric/underscore pattern before query execution, preventing malformed or suspicious table names in `sqlite_master` from being interpolated into raw SQL statements during iCloud restore.

## [1.2.0-beta.2] - 2026-08-28

### Added
- **UI Motion & Animation Polish:** Comprehensive refinement across navigation and interactive components with organic fluid transitions and hardware-accelerated liquid glass transforms:
  - **Universal Card & Container Morphs (`CardMorphRoute`):** High-performance container transform with live liquid glass refraction, iOS edge-swipe back navigation, and seamless card expansion/collapse across Statistics Hub modules, Routine starts, Workout Hub tiles, Diary meal entries, and Floating Action Buttons (`GlassFab`).
  - **Nutrient Value Interpolation (`FoodDetailScreen`):** Smooth tweening (`TweenAnimationBuilder`) when switching between portion and 100g nutrient displays without zero resets.
  ...
  - **Fluid Exercise Removal Animation:** Removing an exercise from a Live Workout (`LiveWorkoutScreen`), Routine Editor (`EditRoutineScreen`), or Workout Log Editor (`WorkoutLogDetailScreen`) now smoothly collapses its card upward (`AnimatedSize`, `Curves.easeInOutCubic`) with an effortless fade-out (`AnimatedOpacity`) instead of popping out abruptly, allowing remaining exercises to seamlessly glide into place.
  - **Live Workout Rest Bar & Action Transitions (`LiveWorkoutScreen`):** When the rest timer starts or expires, the rest bottom bar smoothly slides in and out from below (`AnimatedSlide` & `AnimatedOpacity`, `320ms`, `Curves.easeInOutCubic`) while the floating "Add Exercise" button glides synchronously into place (`AnimatedPositioned`) with zero teleportation. When the countdown completes, the bar morphs with a silky cross-fade (`AnimatedSwitcher`, `280ms`, `Curves.easeInOutCubic`) into the vibrant yellow *"Pause is over"* pill without any hard snaps.
  - **Workout Summary Rolling Numbers (`WorkoutSummaryBar`):** Overall metrics (duration, volume, and sets) organically count up from 0 to their final completed values upon entrance (`TweenAnimationBuilder`, `750ms`, `Curves.easeOutCubic`).
  - **Set Row Expansions & Haptic Checkmark (`LiveWorkoutSetRow`, `WorkoutExerciseLogCard`, `EditRoutineExerciseCard`):** Adding sets smoothly expands the list downward (`AnimatedSize`), and checking off completed sets triggers crisp tactile confirmation feedback (`HapticFeedbackService.confirmationFeedback`).
  - **Diary Meal Accordion Expansion (`MealEntryCard`):** Sub-ingredient dropdown lists expand and collapse with fluid height animations (`AnimatedSize`, `280ms`, `Curves.easeInOutCubic`).
- **Performance Log (`JankRecorder`, `PerformanceDiagnosticsScreen`):** Jank is the one class of bug that cannot be reproduced on a development machine — a Mac renders a screen well inside budget while a three-year-old phone misses it. The app now measures its own frame timings on the device they actually happen on, in release builds, at negligible cost.
  - **Per-Screen Frame Statistics:** `SchedulerBinding.addTimingsCallback` aggregates every frame per screen, keeping build time (Dart work on the UI thread) and raster time (GPU) apart, because the fixes differ: raster-bound jank points at shaders, blur and backdrop filters, build-bound jank at work inside the frame. The budget follows the display's real refresh rate (16.7 ms at 60 Hz, 8.3 ms at 120 Hz), so ProMotion devices are not silently given a doubled allowance.
  - **Freeze Detection:** A 500 ms watchdog records stretches where the UI isolate stopped answering for 1.2 s or more — the "I can't tap anything" case that frame timings miss entirely, since a blocked isolate produces no frames at all. Timer suspension in the background is accounted for, so a resumed app does not report a multi-minute freeze.
  - **Automatic Screen Attribution:** A `NavigatorObserver` derives the screen label from the mounted widget tree, so all ~100 unnamed `MaterialPageRoute` push sites are covered without being touched. Tab switches never reach the navigator, so `MainScreen` names its own route — otherwise the four main tabs, where users spend most of their time, would collapse into a single entry.
  - **Settings → Performance Log:** A live table sorted by share of dropped frames (screens with fewer than 30 frames rank last, where the percentage is noise rather than a measurement), the recorded freezes, and copy / share of the plain-text log. Statistics persist across restarts, since a crash is exactly when the numbers matter.
  - **Feedback Report Section:** An optional "Performance log (frame timings)" section in the diagnostic report, including the device model (`device_info_plus`) — "9.7 % jank" says little without knowing which iPhone produced it.
  - **Localization:** The surrounding UI is localized in English, German, French, Italian and Japanese; the measurements themselves stay English so numbers stay comparable across reports.
- **Telemetry — `performance_stall` (opt-in only, see `TELEMETRY.md`):** A freeze is reported as it happens rather than waiting for a feedback report that may never be sent, carrying the screen's Dart class name, the blocked duration and its bucket, the device model, platform, refresh rate and that screen's frame counters. Capped at five events per app session so a pathological loop cannot flood the pipeline. Nothing is transmitted while telemetry is switched off.
...
- **Performance Data Missing From Direct Feedback Submission:** Enabling the performance section and using "Send directly to developer" listed `performance` in `included_sections` while transmitting none of its numbers. The summary builder parses `key: value` lines out of the report preview and the performance section is a table, so its properties are now taken from the recorder directly. Every submission also reports its `platform`.
...
- **AI Meal & Nutrition Capture (`AiMealCaptureScreen`):** A meal can now be logged from a photo instead of by searching for every ingredient. The capture screen takes up to four photos, an analysis produces the items with their amounts and macros, and a review screen lets each of them be corrected, swapped or removed before the meal is saved.
  - **One Camera for Photo, Barcode and Depth (iOS):** A single `AVCaptureSession` drives the live preview (as a platform view), passive barcode detection (`AVCaptureMetadataOutput`) and the still photo with its depth map. There are no capture modes to choose between: the camera watches for barcodes while the user frames a plate and offers a chip when it finds one. Barcode detection can be switched off next to the shutter — on a plated meal the reappearing chip is noise, on a packaged product it is the point.
  - **LiDAR Scale Calibration (`documentation/features/depth_scale_hint.md`):** On devices with `builtInLiDARDepthCamera`, the capture also yields a hardware depth map and the camera intrinsics (`fx`, `fy`, `cx`, `cy`), from which subject distance and the visible frame dimensions are computed and passed to the model, so portion size is measured rather than guessed. Quality gating (15–120 cm, ≥ 50 % valid points) discards measurements that are not dependable, and distance guidance only appears when the measurement actually fell outside that range. iPhones without LiDAR use the same path and simply get no depth map.
  - **LiDAR Scale Toggle:** "LiDAR-Maßstab senden" in the AI settings, shown only on devices that can measure, so the same meal can be photographed with and without the measurement — otherwise there is no way to tell whether the scale hint improves the estimate at all.
  - **Voice Dictation:** Hold-to-talk dictation via the platform speech recognizer (`speech_to_text`), with a live sound level, a running transcript and an editable result before anything is sent to the AI — dictation mishears numbers, and a silent "150 g" becoming "115 g" would produce a wrong meal. Recognition is attempted on-device first and falls back to the system's network recognizer only when the device cannot do it locally, in which case the sheet says so. Spoken input works instead of a photo (`analyzeText`) or alongside one as a `textHint`, which is the more useful case: cooking method and hidden ingredients are exactly what a photo cannot show.
  - **Hierarchical Meal Entries (`MealEntries`, schema 25):** Drift migration v25 adds a `MealEntries` grouping table with a `mealEntryId` foreign key on `NutritionLogs`, so a scanned meal stays one thing in the diary while its ingredients remain individually editable.
  - **Durable Meal Photo Storage:** Captured photos are copied into the application support directory with a 320 px preview and referenced by *relative* paths. Both matter: the camera hands over files in a temporary directory the system purges, and the iOS container path changes with every app update, so an absolute path in the database is guaranteed to break. Photos are removed together with the meal entry they belong to.
  - **Blocking Analysis Screen (`MealAnalysisScreen`):** The request runs on its own screen rather than under the still-live capture screen, names the current stage, shows no invented progress percentage, and can be left — in which case a late result is discarded rather than pushed at the user. Its organic morphing cloud animation (`AiNeuralCloudOrbWidget`) replaces a `CircularProgressIndicator` with a 60/120 fps hardware-accelerated silhouette with multi-harmonic lobe undulation, ripple cascades and a tap-to-charge dye diffusion, themed for light and dark.
  - **Strict FOSS Compliance:** No proprietary Google Play Services or ML Kit dependencies. iOS barcode scanning uses the system's own `AVCaptureMetadataOutput`; Android keeps the open-source ZXing engine (`qr_code_scanner_plus: ^2.2.0`) and its existing capture path.
  - **Localization:** The capture, analysis, review, meal detail, delete and re-analysis surfaces go through `l10n` in English, German, French, Italian and Japanese — 45 new keys, with proper plurals for ingredient and entry counts rather than glued-together fragments.
- **Interactive First-Time Meal Capture Tour (`AiMealCaptureTourOverlay`):** A step-by-step introduction to the multi-photo shutter, the live barcode scanner (with a focused dummy product preview), gallery import, voice dictation, manual notes and the analysis, over a frosted-glass backdrop with a 10-tier progressive Gaussian blur ramp instead of hard circle borders. An info icon in the app bar replays it at any time.
- **Meal Previews in the Automatic iCloud Backup (`ICloudBackupArchive`):** The iCloud backup used to be the bare SQLite snapshot, so a restored device showed every meal without the photo it was logged with — the rows named their previews, but nothing ever carried the files across. The snapshot now travels inside `icloud_backup.zip` with the previews next to it (`thumbs/meals/`), laid out exactly like a manual `.trainlibre` backup. The database is deflated at best *speed* rather than best size because this also runs unattended when the app goes to the background, where a long CPU burst is the more expensive mistake; the already-compressed JPEG previews are stored uncompressed. Accounts backed up by an older build keep restoring from their bare `icloud_backup.sqlite`, which is deleted from the container once an archive has taken its place so a stale snapshot can never be picked over a current backup. An archive that carries no database is refused rather than restored — swapping the live file for nothing would empty the app.
- **Replay the App Tour From Settings:** An "App-Tour ansehen" action under Support & About repeats the main walkthrough without resetting local app data.
- **Minimized Running Workout Bar (`RunningWorkoutOverlay`):** The bar above the bottom navigation now carries three zones instead of a timer and two labelled buttons: a circular expand affordance on the left, status and context in the middle, and a circular discard action on the right. The middle is two lines — a status dot with `Workout` and the elapsed time, switching to `Rest` and the remaining pause the moment a pause timer runs, and underneath it the exercise the next open set belongs to. Once every set is ticked off it names the last exercise worked; a workout with no exercises leaves the line out entirely rather than showing a placeholder. Everything but the discard button opens the workout; discarding still asks first.
- **Morphing Transition Between the Bar and the Workout Screen (`WorkoutMorphRoute`):** The live workout screen now grows out of the bar and shrinks back into it, in place of a sideways page push. The back arrow in its header is a chevron pointing down, because it no longer leaves the workout — it minimizes it. Four things move at once: the container grows from the pill to the screen with its corners flattening slightly ahead of it, the page is scaled by the container's own width ratio and pinned to its top-left corner so it fills the pill exactly and grows with it, the route draws the bar itself — the same widget the screen renders, not a look-alike — pinned to the same corner and scaled by the same rule so the two sweep together, and the screen left behind dims. Nothing relayouts per frame; the page is only ever transformed, faded and clipped.

### Changed
- **One Media Store for Every Feature (`AppMediaStore`):** Images now live under `media/<feature>/` in the application support directory and travel under `thumbs/<feature>/` inside a backup, instead of in one folder named after the first feature that needed one. The next feature that stores images adds a domain rather than a second, differently shaped photo store, and archives written today keep restoring once it does. `MealPhotoStore` stays as the meal-shaped face of it. Restores from an older archive still take the flat `thumbs/` previews, and orphan sweeping matches by file name rather than by folder — a restored row may name the old folder while its file now lives under the media root, and deleting it because the two differ would be exactly the wrong outcome. Collecting previews for a backup and sweeping orphans after a restore were asked in three places and are now asked in one, which matters most for the query that decides what gets deleted.
- **iCloud Restore Copies Into the Live Database:** The restore used to close the open connection, swap the SQLite file underneath the running app and rely on a restart afterwards. Every data source built in `main` holds that connection object, so from the moment of the swap the diary, the profile and the supplements threw "the connection was closed" while the statistics screen — which resolves its data source through `DatabaseHelper` on each access — kept working, which made it look like the backup had come back empty. The snapshot is now copied into the database the app already has open, table by table, with only the columns both sides share: a backup from an older or newer build restores what it can instead of failing outright, and a table the backup does not carry is left alone rather than wiped, because it belongs to a feature added after the backup was written. Nothing is closed, so onboarding continues against the restored data as it always did — region, catalog download and permissions are still asked.
- **Deleting All Local Data Restarts the App:** The wipe clears every preference and empties every table, but the running app still holds the services, view models and caches that were built from them — and there is no way to hand each of them the news. Both entry points (settings and data management) now rebuild from `main`, which lands on the consent screen by itself, because that is what a device with no preferences is. The same restart helper covers the end of a restored onboarding, and it puts a placeholder on screen while `main` does its asynchronous startup work, so the screen the user just acted on does not sit there looking like nothing happened.
- **Restored Previews Land Where Their Row Says:** A backup carries the previews under one flat name, but the rows it restores carry the path each one had on the device that wrote it — and that is not always this device's layout. A meal logged by a build that stored photos in `meal_photos/` names that folder forever, so writing its preview into `media/meals/` left the row pointing at nothing and the meal came back without its picture. The rows decide now; only a preview no row claims falls back to the feature's own folder.
- **A Kept Generation of the iCloud Backup:** The archive already in the container is moved to `icloud_backup.previous.zip` before a new one takes its place. A restore that goes wrong is followed by the next automatic sync uploading that state over the only copy there is; one spare file in the container is the difference between an annoyance and lost data.
- **iCloud Restore Sweeps Orphaned Photos:** A restore replaces the rows but not the files, so the photos of every meal it overwrites used to stay on disk forever — on a device that restores repeatedly they were the largest thing the app left behind. The file-backup path already swept them; the iCloud path now does too.
- **Diary Rows Share One Shape and One Set of Columns (`DiaryFoodRow`):** Standalone entries were a two-line `ListTile` while meal ingredients were a one-line row with the amount glued to the name — which is why nothing lined up and why a long name could produce `40 g89 kcal`. All diary rows now use `DiaryFoodRow` with fixed amount and energy columns, and every row reserves the same trailing space, so a meal's calorie value lines up with the values above and below it instead of being pushed left by its chevron.
...
- **Performance and GC Optimizations:**
  ...
  - **`ConsistencyTrackerScreen`:** Replaced `.map().fold()` iterable allocation in the build/render path with a direct iteration loop.
  - **`AnalyticsCardBase`:** Streamlined sparkline mini-bar rendering in `buildMiniBars` using single-pass max value extraction and collection-for widget rendering without intermediate list allocations.
  - **`SleepPipelineService` & `SleepDayRepository`:** Replaced $O(N \times M)$ nested lookups with Set-based ID lookups, streamlined sleep interruption calculations into a single pass, and eliminated intermediate lists when mapping resting heart rates.
  - **`PulseAnalysisEngine` & `PulseRepository`:** Replaced `DateTime` allocations and closure-allocating `putIfAbsent` calls in raw pulse sanitization with epoch integer arithmetic and direct map lookups; replaced `.skip().take()` downsampling with an index-based sub-loop.
- **Accessibility & Semantics Enhancements:**
  - **`SetTypeChip`:** Added localized accessibility announcements (`Semantics(button: true)`) for workout set types (`set_type_normal`, `set_type_warmup`, `set_type_failure`, `set_type_dropset`).
  - **`TimeRangeFilter`:** Added localized `previousPageTooltip` and `nextPageTooltip` tooltips to date range navigation chevrons.
  - **Main Screen FAB & Speed Dial:** Added localized semantic labels (`addMenuTitle`, `closeButtonTooltip`) to the floating action button and speed dial overlay.
- **Privacy-Preserving Telemetry Extensions:** The opt-in, zero-PII schema now covers voice dictation (`voice_dictation_used`, `voice_dictation_completed`, with bucketed durations), multi-modal meal capture (`ai_meal_scan_requested`, `ai_meal_scan_completed`, with modality flags, photo counts and latency buckets), LiDAR depth (`lidar_depth_captured`, `lidar_depth_visualized`), review corrections (`ai_meal_correction_submitted`, `ai_meal_correction_completed`) and offline OpenFoodFacts catalogs (`off_catalog_installed`, `off_catalog_updated`), plus screen views for `meal_analysis` and `voice_dictation_settings`. No spoken words, recordings, transcripts, user text, food names or location data are captured or transmitted.
...
- **Floating Bottom Bars Share One Set of Tokens (`DesignConstants`):** The workout bar's pill geometry, fill and typography now come from the same constants as the bottom navigation's selection indicator (`floatingBarPillInset`, `floatingBarPillSize`, `floatingBarPillColor`, `floatingBarLabelStyle`) rather than from values of their own, so the two bars sitting one above the other read as one system.
- **iOS Scroll Physics Completed (`NoGlowScrollBehavior`):** `BouncingScrollPhysics` now wraps `RangeMaintainingScrollPhysics`, which is what Flutter's own iOS default does. Without it a list jumps when something above the viewport changes size — a chart finishing its load, a card expanding — instead of holding the reader's place. The deceleration itself was already native: `BouncingScrollPhysics` defaults to the same 0.998 as `UIScrollViewDecelerationRateNormal`.
...
- **The Workout Bar No Longer Blinks Out as the Morph Lands:** The transition draws its own copy of the bar and the screen hides the original for the length of the flight. Handing back on the route's disposal meant the copy was already gone while the screen had not rebuilt its bar yet, so for one frame there was no bar at all — a flicker at exactly the moment the animation arrived. The hand-back now happens when the collapse lands, while the copy is still on screen at precisely the bar's position, size and opacity, so the two overlap indistinguishably instead of leaving a gap. The opening hand-over is deferred by one frame for the same reason, since a route's content is not in the tree until the frame after the push.
- **Liquid Glass No Longer Snaps In During the Morph:** Glass is a `BackdropFilterLayer`, and such a layer can only refract what was painted into the save layer enclosing it. An `Opacity` is exactly that — an empty one — so the fading bar had nothing to refract and rendered flat until its opacity reached 1.0, at which point Flutter stops pushing the layer and the glass appeared from nothing. The bar is now always drawn at full opacity with the page fading over it, and the area of the container below the bar is filled in the page's own background colour so the screen behind cannot wash through while the page is still arriving.
