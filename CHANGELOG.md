...
- **AI Meal & Nutrition Capture with LiDAR Depth Scaling (`Mahlzeiten-Erfassung`):**
  ...
  - **Organic Morphing Cloud Loading Animation (`AiNeuralCloudOrbWidget` & `MealAnalysisScreen`):** Replaced placeholder `CircularProgressIndicator` with a high-performance 60/120fps hardware-accelerated organic thinking cloud animation inspired by modern AI voice/thinking silhouettes. Features an interconnected multi-lobe morphing cloud with continuous gentle rotation, harmonic lobe breathing, a detached trailing satellite bubble at the bottom-left, subtle ambient back-glow, and a clean red cancel action.
