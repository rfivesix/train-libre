...
- **AI Meal & Nutrition Capture with LiDAR Depth Scaling (`Mahlzeiten-Erfassung`):**
  ...
  - **Interactive First-Time AI Meal Capture Tour (`AiMealCaptureTourOverlay` & `AiMealCaptureTourService`):** Step-by-step interactive onboarding tour that automatically introduces first-time users to the multi-photo shutter, live barcode scanner (with simulated dummy product preview), gallery import, voice dictation, manual text notes, and smart AI analysis. Includes a dedicated info icon in the top app bar to replay the guided introduction anytime.
