...
- **Macronutrient Analytics & Statistics Hub Integration (`StatisticsHubScreen`, `MacroSectionCard`, `MacroStatisticsScreen`, `MacroHistoryStackedBarChart`, `MacroAnalyticsDataAdapter`):** Added a new dedicated "Ernährung" (Nutrition) analytics section to `StatisticsHubScreen` positioned below Training. Features the interactive `MacroSectionCard` preview displaying the last 7 days of macronutrient distribution (stacked vertical bars of Carbs, Fat, and Protein with total height proportional to daily calories) and average calories (`XXXX kcal >`). Tapping navigates to `MacroStatisticsScreen`, complete with timeframe filters (`1W`, `1M`, `3M`, `6M`, `1Y`, `Max`), a 4-metric summary header (daily averages for Calories in Orange, Protein in Red, Fat in Purple, Carbs in Green), the vertically elongated stacked bar chart with horizontal caloric reference grid lines (0, 2000, 4000 kcal), interactive segment selection, and a chronological daily breakdown list showing percentage ratios (`% P • % F • % C`) and absolute gram intake.
- **7-Day Intake vs. Target Comparison Chart (`NutritionHubScreen`, `NutritionRecommendationCard`, `WeeklyIntakeVsTargetChart`):** Integrated the interactive 7-day intake comparison chart into the adaptive recommendation card inside the Nutrition Hub. Displays 7 vertical stacked bars for actual intake (Monday–Sunday / past 7 days) with total calories badge and macro tags (`P`, `F`, `C`), separated by a clear visual divider gap from an 8th target bar ("Ziel") showing recommended target calories and macros from the active recommendation, enabling horizontal visual comparison between actual eating habits and adaptive targets.
