...
- **Recovery tracker planning:** Added a separate v2 implementation plan for a time-decaying, per-set recovery-load model. It retains different baseline recovery kinetics by muscle group, preserves the existing model until replacement is validated, and records the required evidence boundaries and regression scenarios.
