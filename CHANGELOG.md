...
- **Onboarding Route Scope & Completion State Protection:** Wrapped `OnboardingScreen` in `PopScope` to redirect back gestures to `_prevPage()`, preventing accidental screen disposal on Android/iOS. Fixed `_runAutomatedPermissionSequence` to explicitly set `_onboardingCompletedSuccessfully = true` and track `trackOnboardingCompleted` before app restart, eliminating premature `onboarding_abandoned` event triggers and ensuring `hasSeenOnboarding` is only marked when onboarding is fully completed.
...
- **Speed Dial Menu FAB Alignment & Instant Handoff:** Adjusted the bottom offset of `SpeedDialMenuOverlay` (`speed_dial_menu_overlay.dart`) from `32.0px` down to `16.0px` (and action list offset to `86.0px`), perfectly matching the bottom navigation bar FAB button. Implemented instant FAB handoff on button press (`_menuController.value > 0.0`) in `main_screen.dart`, hiding the underlying navigation bar FAB as soon as touched so only a single glass shader container is rendered during the entire rotation and sprout animation.
