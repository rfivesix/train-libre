...
- **Diary date strip:** The Diary header now offers a horizontally scrollable,
  ...
  layout aligns the selected date as the final fully visible day, marks today
  with an accent outline, uses a compact summary-card surface for the calendar,
  and softly fades the scrollable strip at its edges. The strip ends at today;
  selecting a date automatically restores the intended alignment.
