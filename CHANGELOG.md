...
- **Diary Card Visual Alignment & Typography:**
  ...
  - Integrated `WeightCard` directly into the diary skeleton stream so that on empty historical days, it shimmers naturally within the background skeleton under `ActiveGapOverlay` rather than floating above the "No data available" overlay, while preserving the live interactive card on today.
...
- **Repository Hygiene & Public Open-Source Sanitization:**
  ...
  - **Documentation Website Navigation:** Removed the "Android APK" header navigation link across all documentation pages and cleaned up the corresponding i18n keys. Repositioned the GitHub link to the right of the language and theme icons on mobile/tablet screens and restored the "Train Libre" brand title next to the app icon.
...
- **Weight Card Text Hierarchy (`WeightCard`):** Present “Weight” as a standard card title, keep the unit visually attached to the value with identical typography, and reserve the smaller muted style for the measurement age.
...
### Fixed
- **Automatic Bundled Catalog Upgrades & Reliable Downloads (`BasisDataManager`, `ExerciseCatalogRefreshService`):** App startup now imports a newer bundled exercise catalog offline even when an older catalog is already installed. Manual updates retain the installed version, check the latest release before selecting a cached download, and never replace newer installed data with an older bundle, cache or remote release. Valid newer cached downloads remain available when the network or download validation fails. Remote validation now accepts OpenExerciseDB's relational translation table as well as legacy inline translations.
