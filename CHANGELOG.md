...
- **App Tour Highlight Sizing & Label Inclusions:** Fixed spotlight rectangle sizing on Step 1 ("Hauptnavigation") by binding `_tourNavigationBarKey` directly to `GlassTabBar.bottom` instead of the outer layout stack, preventing the highlight box from expanding beyond screen boundaries. Extended `_rectForKey` calculations for individual tab steps (`_tourDiaryTabKey`, `_tourWorkoutTabKey`, `_tourStatisticsTabKey`, `_tourNutritionTabKey`) to encompass both the tab icon and its text label below. Reduced overlay spotlight inflation padding (`targetRect?.inflate(4)`) for crisp, tight rounded highlight borders. Updated `_skipAppTour` and `_completeAppTour` to navigate back to the **Tagebuch** tab (`_onNavigationTapped(0)`).
