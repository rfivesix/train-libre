...
### Added
- **Long-Term Goal Tracking & Engine Integration (`MyGoalsScreen`, `GoalDetailScreen`, `CreateGoalFlow`):** Introduced first-class nutrition goal tracking adhering to the Single Active Goal rule. Includes a guided 7-step goal creation wizard with automatic baseline detection from weight logs, target date/rate calculations with physiological limits (0.25–1.0 kg/week; 0.5–1.5% body weight/week), motivation capture, and lifecycle management (`superseded`, `retired`, `resumed`).
- **Goal Trajectory & Visual Progress (`GoalProgressHeroCard`, `GoalAdjustmentSheet`):** Added a visual goal dashboard card showcasing current vs. baseline weight, remaining delta, progress percentage bar, dynamic target pace, and projected completion date. Users can perform trajectory adjustments via bottom sheet, archiving the predecessor goal and generating an event-linked successor.
