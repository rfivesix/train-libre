...
- **AI Meal & Nutrition Capture with LiDAR Depth Scaling (`Mahlzeiten-Erfassung`):**
  ...
  - **Interactive First-Time AI Meal Capture Tour (`AiMealCaptureTourOverlay` & `AiMealCaptureTourService`):** Step-by-step interactive onboarding tour that automatically introduces first-time users to the multi-photo shutter, live barcode scanner (with focused simulated dummy product preview), gallery import, voice dictation, manual text notes, and smart AI analysis. Replaced prominent circle borders with a clean, subtle frosted-glass backdrop blur (`ImageFilter.blur(sigma: 14)`) that keeps active targets crisp while blurring the background. Includes a dedicated info icon in the top app bar to replay the guided introduction anytime.
  - **Main App Walkthrough Tour Modernization (`AppTourOverlay` & Settings Replay):** Upgraded the primary app walkthrough with the modern frosted-glass backdrop blur (`ImageFilter.blur(sigma: 14)`), soft feathered edges (`_FeatheredEdgePainter`), pill progress indicators, accurate full-size element cutouts (FAB, navigation bar, and tab indicators), and smooth bottom-pinned card transitions without upward jump flickers. Added a dedicated "App-Tour ansehen" action in Settings under Support & About to easily replay the walkthrough anytime without resetting local app data.
