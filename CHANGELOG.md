...
## [1.0.2-alpha.1] - 2026-08-10

### Added
- **iOS Lock Screen Widgets (.accessoryInline, .accessoryCircular, .accessoryRectangular):** Added native iOS Lock Screen accessory widget support across `TodayGlanceWidget` (Nutrition: inline compact calories & protein without icons/emojis, circular gauge with `Protein` label underneath, rectangular calories & protein breakdown without titles or trailing percentages), `RecoveryWidget` (Muscle Readiness: inline compact localized text e.g. `0 Ready · 10 Rec. · 3 Fresh` / `0 Ber. · 10 Erh. · 3 Fr.`, rectangular 3-row stacked state layout with monospaced right-aligned numbers for perfect horizontal label alignment), and `QuickActionsWidget` (1-Tap Circular Action Buttons configurable for Barcode Scanner, AI Meal, Start Workout, Add Water, etc.). Built with native SF Symbols, full EN/DE/FR/IT/JA localization, system vibrancy, and instant deep-linking.
- **"Lebensmittel hinzufügen" Quick Action & Shortcut:** Added `add_food` ("Lebensmittel hinzufügen") to iOS Home Screen Quick Actions Widget, App Shortcuts (Apple Shortcuts app & Siri), and iOS 18 Control Center / Lock Screen / Action Button controls.
- **iOS App Shortcuts, Control Center Controls (iOS 18), Lock Screen & Action Button Integration:** All 7 quick actions (`Add Food`, `AI Meal`, `Scan Barcode`, `Start Workout`, `Add Measurement`, `Log Supplement`, `Add Liquid`) are now available as native iOS App Shortcuts (in the Apple Shortcuts app, Siri voice triggers, and Spotlight search), Control Center buttons (`ControlWidgetButton` on iOS 18+), Lock Screen bottom controls, and hardware Action Button mappings (iPhone 15 Pro / 16). Implemented via `QuickActionIntents.swift` using `AppShortcutsProvider` and `ControlWidgetConfiguration`.
- **The Live Workout Screen Opens at the Exercise That Is Up Next:** Entering the screen — from the workout hub or by tapping the Live Activity — scrolls the exercise holding the next open set to the top of the list, directly beneath the duration/volume/sets bar, instead of always starting at the very beginning of a long routine.

### Fixed
- **Improved Inner Tile & Heatmap Card Contrast for Light Mode Widgets:** Adjusted `secondarySurfaceLight` in `StatsWidgetChrome.swift` from `#F2F2F7` to `#E4E6EB`. Inner metric tiles (Duration, Volume, Sets) and the muscle heatmap container now cleanly separate from the widget background in Light Mode without sacrificing contrast against the muscle model silhouette.
- **Direct Barcode Scanner Launch Without Delay:** Tapping "Scan Barcode" now directly launches `ScannerScreen` without opening an intermediate screen or waiting for frame animation callbacks.
- **Today's Glance Dark Mode & iOS 18 Tinted / Glass Mode Contrast:** Elevated unfilled progress bar backgrounds in Dark Mode with a 0.8pt glass border (`.stroke(Color.white.opacity(0.18))`), replaced `.widgetAccentable()` on custom hex fills with translucent tinted capsules (`Color.primary.opacity(0.35)`), and ensured text inside progress bars remains 100% readable in iOS 18 Tinted and Glass (Clear) appearance modes.
- **App Shortcuts Deep Link Execution & iOS Simulator Support:** Isolated `TrainLibreAppShortcuts: AppShortcutsProvider` strictly to the main app target (`ios/Runner/TrainLibreAppShortcuts.swift`), preventing `appintentsd` daemon registry corruption on iOS Simulator, and updated AppIntent runtime launcher to invoke `openURL:options:completionHandler:` on `UIApplication.shared`.
- **Values Typed Into a Set Were Overwritten by the Routine's Plan:** `LiveWorkoutViewModel` and the Live Activity's content builder both read `template.target… ?? log.…`, giving the routine's planned numbers precedence over what the user had actually entered. Two consequences: a set logged as 24 kg × 10 kept showing the template's 10 kg × 10 outside the app, and completing that set from the Live Activity wrote the template values back over the entered ones — a silent data loss, not just a display fault. The precedence is now reversed for weight, reps and RIR: what is logged wins, the plan only fills in for sets that have not been touched yet.
- **Exercise Names Were Always Shown in English Outside the App:** `SetLog.exerciseName` stores the English name as its lookup key and must never be displayed, but the Live Activity used exactly that. `Exercise` only offered `getLocalizedName(BuildContext)`, which the context-free content builder cannot call, so it silently fell back to English for every language. Added `Exercise.localizedNameFor(String languageCode)` — `getLocalizedName` now delegates to it, keeping one implementation — and the builder resolves the name from the language tag it already receives.
- **A Second Live Workout Screen Was Pushed on Every Live Activity Tap:** The deep link went through `onGenerateRoute` unconditionally, so each tap stacked another copy of the screen; going back walked through all of them. `_MyAppState` now intercepts the platform route push and pops back to the existing screen when one is already on the stack.
- **Rest Bar Stayed Behind the Keyboard:** The floating rest bar remained visible when the software keyboard opened, ending up underneath it. It is hidden while the keyboard is up, matching the add-exercise button — both now share one `_HiddenWhileKeyboardVisible` wrapper rather than two separate keyboard observers.
- **Live Activity Version Mismatch Blocked App Store Uploads:** The `TrainLibreLiveActivity` extension kept Xcode's template defaults (`MARKETING_VERSION = 1.0`, `CURRENT_PROJECT_VERSION = 1`) while the app ships `1.0.0` / `1000013`. App Store Connect rejects any upload whose extension version does not match its containing app, so the first archive would have failed validation. The extension now tracks the app's values.
- **Quick Action Home Screen Widget Did Not Adapt to Configured Actions:** The editor accepted a new action but WidgetKit always sent the parameter defaults to the timeline provider. The generated metadata proved that the fields were exported correctly; the fault was iOS 18 restoring four independent parameters that all referenced the same `AppEnum` type. Each slot now has its own stable AppEnum type and converts to the common rendering action only inside the provider. This gives the system an unambiguous persistence key for every selection.
- **Crash-Class Assertion When Leaving the Live Workout Screen:** `dispose()` called `ModalRoute.of(context)`, an inherited-widget lookup on an already deactivated element, which threw *"Looking up a deactivated widget's ancestor is unsafe"* on every exit. The route is captured while the element is still active and compared by reference instead.

## [1.0.2-alpha.1] - 2026-08-10

### Added
- **Live Activity for Workouts (iOS 16.2+):** While a workout is running, a Live Activity on the Lock Screen and in the Dynamic Island (iPhone 14 Pro and newer) shows what is up next — exercise, set type, position, weight and reps or RIR — and the rest countdown, without unlocking the phone or opening the app. Tapping it returns to the live workout screen. On iOS 17+ the card carries working `−15s`, `+15s`, `Skip` and complete-set controls. All four presentations are covered (Lock Screen banner, Dynamic Island expanded, compact, minimal); with a second Live Activity on screen ours is reduced to the minimal circle, which then shows the shortest truthful form of the set (`115×9`) rather than the app icon. Cardio reuses the same layout with duration, distance and RPE in place of weight, reps and RIR. Everything the extension displays is formatted in Dart before it crosses the platform channel (`buildWorkoutLiveActivityContent`) — unit conversion, number formats, localization (DE/EN/FR/IT/JA) and set-type badge colors mirroring `SetTypeChip` — so the Swift side never formats a value itself. Times travel as absolute `Date`s rather than counters, letting SwiftUI animate them without an update per second; a running workout needs roughly one push per set. No APNs and no server: every update is local, in keeping with the app's offline-first design. New iOS extension target `TrainLibreLiveActivity`, shared model and MethodChannel bridge in `ios/LiveActivity/`, hand-rolled rather than taken from a plugin because the payload is entirely workout-specific. Requires the `group.com.rfivesix.trainlibre` App Group, which the widget's App Intents use to hand set completions back to the app. Covered by `test/features/workout/live_activity_content_test.dart`.
- **Set completion is blocked when a set has no values:** A set with no weight or reps (no duration or distance for cardio) cannot be ticked off from the Live Activity. The checkmark turns grey and stops being a button; tapping it opens the app so the numbers can be entered. Missing values are shown as `–` rather than being filled in from the plan, and `LiveWorkoutViewModel` refuses the same completion server-side, so a command queued before the values were cleared cannot silently write a wrong set.
- **Rest sound is rescheduled when the pause is changed from outside the app (iOS):** While a Live Activity is running, the "rest is over" sound is scheduled natively via `UNUserNotificationCenter` instead of `flutter_local_notifications`. The widget's App Intents run in their own process without a Flutter engine, so only a native schedule can be moved along when `+15s` or `Skip` is used while the app is suspended — otherwise the sound would have fired at the original time. Without a Live Activity, and on Android, the existing path is unchanged. The redundant banner is suppressed while the Live Activity is visible; sound and haptics still fire.

### Fixed
- **A Rest Timer Kept Running After Completing a Set of a Pause-less Exercise:** `LiveWorkoutViewModel.updateSet` only ever *started* a rest timer, when the completed set's exercise had a pause configured and the set was not the last one of that exercise. In every other case it did nothing at all — so a pause still counting down from a previous exercise survived untouched. Completing a set of an exercise without a configured pause (or the final set of one) therefore left the earlier exercise's timer running and visible, both in the live workout screen and, since this release, in the Live Activity. The pause belonging to the set that was just completed is now authoritative: if it defines none, any running pause is cancelled.
