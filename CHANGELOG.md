...
### Changed
- **Diary Meal Photo Backdrop Alignment:** Centered meal photo thumbnail with bilateral horizontal gradient fading to transparent on both left and right edges, keeping meal titles and calorie counts cleanly legible and flush across all rows.
- **Unified Review Suggestions & Meal Detail Design:**
  ...
  - Full-bleed photo extension behind `GlobalAppBar` without corner radius clipping artifacts in Light Mode and consistent top/bottom fade gradients for RGB photos and LiDAR elevation heatmaps alike.
- **4:3 Camera Viewfinder & Theme Adaptation (`AiMealCaptureScreen`):** Framed the live camera viewfinder in standard 4:3 (3:4 portrait) aspect ratio to match native camera sensor hardware and prevent distortion, with theme-adaptive controls, background, LiDAR badge, and barcode banner across Light and Dark themes.
