...
## [1.0.2-alpha.1] - 2026-08-09

### Added
- **Live Activity for Workouts (iOS):** While a workout is running, a Live Activity on the Lock Screen and Dynamic Island (iPhone 14 Pro+) shows the current or next set — exercise, set type, weight/reps or RIR, and the rest timer — without unlocking the phone or opening the app. Covers five states (set pending, resting, rest overdue, no sets left, workout empty) across all four Dynamic Island presentations (banner, expanded, compact, minimal), documented with the full state matrix in `documentation/features/live_activity_workout.md`. All text and numbers are pre-formatted in Dart (`buildWorkoutLiveActivityContent` in `lib/features/workout/domain/live_activity/`) — unit conversion, localization and set-type badge colors (mirroring `SetTypeChip`) all happen before the payload crosses the platform channel, so the Swift side never formats anything itself. Rest countdowns use SwiftUI's `Text(timerInterval:)`/`ProgressView(timerInterval:)` driven by absolute `Date`s rather than a value pushed every second, and the overdue state is derived locally from `staleDate`/`context.isStale` rather than requiring a push at the exact moment the app is typically suspended. Interactive buttons (iOS 17+: −15s/+15s/Skip/complete-set) run as `LiveActivityIntent`s in the widget extension's own process; since they cannot reach the app's Drift database directly, they queue commands in the shared App Group (`group.com.rfivesix.trainlibre`) that `LiveWorkoutViewModel.applyPendingLiveActivityCommands()` drains and applies the next time the app resumes. Cardio sets reuse the same states with a duration/distance/RPE metrics line instead of weight/reps/RIR and no set-type badge; `SetTemplate` currently has no planned-duration/-distance fields, so a fresh cardio set shows only its position until a value has been logged (tracked as a follow-up). New iOS extension target `TrainLibreLiveActivity` (`ios/TrainLibreLiveActivity/`) plus a shared `WorkoutActivityAttributes` model and `WorkoutLiveActivityBridge` MethodChannel handler in `ios/LiveActivity/`, deliberately hand-rolled rather than via a Live Activity plugin given how workout-specific the payload is. Covered by `test/features/workout/live_activity_content_test.dart` (state transitions, set-type badges, cardio formatting).

### Fixed
- **iOS Live Activity Spec v1 Design Alignment & Deep Link Routing:** Adapted Lock Screen banner, Dynamic Island Expanded, Compact, and Minimal layouts to 1:1 match `Live Activity.pdf` (Spec v1). Replaced placeholder SF Symbol dumbbell icon with official Train Libre app logo (`LiveActivityIcon` vector asset). Fixed interactive button touch handling (`−15s`, `+15s`, `Skip`, checkmark) by registering `TrainLibreLiveActivity` synchronized root group in `Runner` target's `fileSystemSynchronizedGroups` in Xcode's `project.pbxproj`, allowing `appintentsmetadataprocessor` to register `LiveActivityIntent`s in the app bundle while configuring `membershipExceptions` to keep `WorkoutLiveActivityWidget.swift` and `WorkoutLiveActivityViews.swift` isolated to the extension target. Fixed compact Dynamic Island timer visibility by removing `.fixedSize()` constraint from `compactLeading` slot, allowing `Text(timerInterval:)` to render live countdown values (`1:26`, `0:45`) in bright accent color. Fixed rest timer expiry transition by evaluating `Date() >= restEndsAt` locally, ensuring SwiftUI immediately transitions from resting state S2 to overdue state S3 (`LAOverdueLabel` + checkmark button) when rest ends instead of lingering on `0:00`. Set-type badges (`W`, `F`, `D`, `3`, `S`, `O`) lead the metrics line inline (e.g. `W 72,5 kg × 8 Wdh (RIR 2)`), with normal sets showing set numbers (`3`) in neutral white (`#FFFFFF`). Added 3-pt hairline progress bar matching control button width, un-dimmed metrics row for resting state S2 (bright white exercise name and set parameters), and checkmark button with red `seit 0:49 überfällig` (`#FF6B78`) for overdue state S3. Fixed compact island right-curve clipping by adding 8-pt trailing padding to compact secondary reps (`× 10`). Fixed top trailing workout duration timer (`34:07`) right alignment in expanded Dynamic Island, and removed root `.widgetURL` from `DynamicIsland` so interactive buttons receive direct touch events. Added `onUnknownRoute` & `onGenerateRoute` handlers to `MaterialApp` in `lib/main.dart` so tapping the Live Activity widget deep link (`trainlibre://workout/live`) opens `LiveWorkoutScreen` directly when a workout is active, and tapping state S4 "Übung hinzufügen" (`trainlibre://workout/live?action=add_exercise`) automatically pops open `ExerciseCatalogScreen` in selection mode. Fixed active exercise card auto-scroll in `LiveWorkoutScreen` by attaching `_exerciseCardKeys` to the outermost container of each exercise card (wrapping `WorkoutCard`), ensuring `Scrollable.ensureVisible` positions the exercise title header ("Preacher Curl") directly below `WorkoutSummaryBar` instead of scrolling the title off-screen. Fixed `WidgetRenderer_Activities` crash by replacing circular `ProgressView` in 12pt minimal slot with SF Symbols. Added missing `NSSupportsLiveActivities` keys and resolved Xcode target entitlements.
