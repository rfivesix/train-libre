...
- **Release Metadata & App Store Review Information:** Authored localized 1.4.1 user-facing release notes and regenerated the in-app "What's New" catalog across all 6 locales (`metadata/whats_new/`, `whats_new_content.g.dart`, `metadata/app_store/`), refreshed TestFlight testing notes (`what_to_test.txt`), and updated App Reviewer testing instructions (`notes.txt`) with an OpenAI API key placeholder and model recommendation (`gpt-5.6-sol`) for testing optional AI features.
