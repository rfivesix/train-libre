...
### Added
- **PostHog Telemetry Expansion for Training & Nutrition Plans (`TelemetryService`, `TELEMETRY.md`):** Extended anonymous usage telemetry to fully cover the new Training Plans and Nutrition Goals / Adaptive Plans subsystems under strict zero-PII safeguards. Added new screen views (`training_plan_hub`, `training_plan_editor`, `my_goals`, `create_goal_flow`, `goal_detail`, `weekly_goal_review`), feature keys, and dedicated events (`training_plan_created`, `training_plan_updated`, `training_plan_toggled`, `training_plan_deleted`, `training_plan_session_started`, `nutrition_goal_created`, `nutrition_goal_adjusted`, `nutrition_goal_retired`, `weekly_goal_review_completed`). Enhanced `workout_completed` with `from_training_plan` to compare adherence rates between planned and ad-hoc sessions. All payloads strictly filter personal text, routine titles, body weight values, and caloric figures into coarse buckets and sanitized standard enums.
- **Focused onboarding profile steps and identifiable funnel events (`OnboardingScreen`, `profile_slide`, telemetry):** Split name, age/gender, and height into individual onboarding screens. Height now uses a tactile vertical ruler, while weight and body-fat remain together. `onboarding_step_viewed` events now include an explicit `screen_name` so PostHog funnels can identify the displayed screen unambiguously.
- **Onboarding input polish (`profile_slide`, `OnboardingMeasurementsStep`):** Made the name step more welcoming with an optional profile-photo picker, added an explicit date placeholder, improved the height ruler with primary-color feedback and labelled major ticks, and introduced the established ruler picker for weight plus an optional expandable body-fat ruler.
- **Smooth ruler input and compact measurement editing (`AppRulerPicker`, `OnboardingMeasurementsStep`):** Rulers now move continuously and snap only after release. The onboarding measurement step replaces duplicate text fields with a prominent value plus an optional pencil editor that synchronizes back to its ruler; body-fat ruler spacing now matches the weight ruler and its expansion styling remains neutral.
- **Inline onboarding measurement editing (`OnboardingMeasurementsStep`):** Replaced the separate manual-entry dialog with an inline editor at the highlighted value. Body-fat now labels every whole percentage point while retaining the same visual tick density as weight.
- **Activity-baseline guidance (`PriorActivityHelpBlock`):** Presented the activity-level explanation as a recognizable info card with a leading information icon, making its relationship to the baseline daily-activity choice clearer.
- **Onboarding Nutrition & Bodyweight Goal Flow Integration (`OnboardingScreen`, `GoalFlowState`, `create_goal_flow`):** Integrated the guided 6-step nutrition and bodyweight goal wizard directly into the user onboarding sequence. The flow establishes prior activity and extra cardio before presenting the goal decision ("Jetzt einrichten" vs. "Später einrichten"), synchronizes baseline weight bidirectionally with onboarding measurements, defaults safely to neutral maintenance when skipped, computes dynamic adaptive nutrition recommendations based on baseline and activity, and persists exactly one active goal without duplicates from `LegacyGoalMigration`. Reusable goal step widgets (`GoalPresetStep`, `GoalTargetStep`, `GoalPaceTimelineStep`, `GoalMotivationStep`, `GoalReviewStep`) were extracted to share clean state between onboarding and the standalone profile goal creation flow with no nested progress bars.
- **Adaptive Training Plan Schedule Grid (`ManualPlanScreen`, `PlanOverviewCard`, `manual_plan_ui`):** Added a permanent overview card directly beneath the training plan header displaying the complete 7-day weekly schedule or up to 14-day training sequence in an adaptive 2-column grid without horizontal scrolling. Visually differentiates workout routines from rest days, highlights the active session slot, respects reduced motion settings, and includes a direct "Bearbeiten" action opening `ManualPlanEditorScreen` via `CardMorphRoute`.
- **Completed planned-workout details:** Completed and partially completed sessions in a training plan now provide a direct action to open their saved workout details.
- **Always-visible training-plan overview:** The normal training-plan screen now shows the complete weekly schedule or sequence in the app's compact, localized two-column value grid above the dated workout view. It distinguishes routines from rest days and highlights the current or next relevant plan slot.
- **Workout Hub Overhaul & Hevy-style Routines (`WorkoutHubScreen`, `Routines`, `watchAllRoutinesWithDetails`):** Redesigned the Workout Hub to serve as the unified, calm, and high-quality central workspace for training. Retained the active training plan hero card (`WorkoutPlanHeroCard`) and muscle recovery / readiness tracker (`RecoverySectionCard`) as top anchors. Replaced the oversized quick-start card with a compact 2-column duo-row (`+ Freies Training` and `+ Neue Routine`). Routines are now displayed in a direct vertical list ordered strictly by most recent usage (`lastUsedAt` descending, updated upon routine creation, saving edits, or starting workouts). Routine cards adopt a clean Hevy-inspired design: bold title with a 3-dots overflow menu (Duplicate, Share, Delete), an ordered comma-separated exercise summary truncated after two lines with an ellipsis, and a full-width primary "Workout starten" button. Tapping the card body opens the routine editor.
- **Global App Bar Workout History Action (`MainScreen`, `GlobalAppBar`):** Added a dedicated workout history action button (`LucideIcons.rotate_ccw_clock`) to the global top app bar on the Workout tab next to the profile avatar, keeping workout history immediately accessible without cluttering the hub surface.
- **Exercise Catalog Navigation Tile (`WorkoutHubScreen`):** Replaced the prominent exercise catalog section with a quiet, low-friction navigation tile at the bottom of the Workout Hub above the navigation bar spacer.
- **Reactive Multi-Table Query (`watchAllRoutinesWithDetails`):** Implemented an efficient single joined reactive Drift stream across routines, routine exercises, exercises, and translations to eliminate N+1 queries and reactively reflect data and language updates in real time.
- **Workout Plan Reminders:** Added opt-in local reminders for active manual plans with a configurable time, automatic rescheduling after plan changes, no catch-up prompts, and direct notification navigation to the plan. The general notification settings now clearly separate workout reminders from goal and review notifications.
- **Manual Workout Plans:** Added user-authored weekly plans and 1–14 day training sequences built from saved routines, with rest days, a browsable calendar, direct Diary starts, explicit skips, partial/completed tracking, one-active-plan handling, immutable plan revisions, routine snapshots, resume/restart behavior, and backup/restore support. Weekly plans keep fixed weekdays, while missed sequence workouts remain next until completed or skipped.
- **Nutrition Developer Lab (`DeveloperSettingsScreen`, `NutritionDeveloperLabView`, `NutritionSandboxWidget`):** Added a dedicated interactive testing lab inside Developer Settings featuring dual tabs ("Allgemein" and "Nutrition Lab"). Includes an interactive live sandbox with reactive sliders (baseline, target, current weight, elapsed/planned weeks, recent/operating rate, weigh-in counts, logged intake days, calorie targets, TDEE) calculating the adaptive engine state matrix in real-time, live previews of embedded cards and direct launch into `WeeklyGoalReviewScreen`, 8 canonical real-world scenario presets (Plateau, Intake Deviation, On Track, Catching Up, Too Aggressive, Insufficient Data, Target Date Passed, Target Reached) with one-tap in-memory preview and database fixture seeding/cleaning (`NutritionTestDataSeeder`), and local notification triggers for testing lock-screen alerts and payload navigation.
- **End-to-End Weekly Goal Reviews & Notifications:** Added idempotent scheduler-aligned weekly review generation (including calibrating reviews for insufficient data), database-enforced window uniqueness, lifecycle cleanup for superseded/retired goals, stable per-goal target-date schedules, preference-aware adaptive notifications, structured notification payloads, and safe warm/cold-launch routing into the Nutrition Hub, goal details, or an open weekly review.
- **Long-Term Nutrition Goal Tracking (`MyGoalsScreen`, `GoalDetailScreen`, `CreateGoalFlow`):** Introduced first-class nutrition goal tracking adhering to the Single Active Goal rule. Includes a guided 6-step goal creation wizard with automatic baseline detection from weight logs, interactive bidirectional pace & target date planner with physiological limits (0.25–1.0 kg/week; 0.5–1.5% body weight/week), motivation capture with quick suggestion chips, and lifecycle management (`active`, `superseded`, `retired`, `resumed`).
- **Goal Trajectory & Visual Dashboard (`GoalProgressHeroCard`, `GoalAdjustmentSheet`):** Added a visual goal dashboard card showcasing current vs. baseline weight, remaining delta, progress percentage bar, dynamic target pace, and projected completion date. Weekly reviews now expose trajectory adjustments directly in the review screen; ordinary plan revisions preserve the existing goal, baseline, measurements, and progress history.
- **Weekly Goal Review System (`WeeklyGoalReviewScreen`, `AdaptiveReviewCard`):** Implemented systematic 7-day review gates requiring $\ge 3$ weigh-ins and $\ge 4$ calorie logs for data sufficiency, plus $\ge 0.15$ kg/week deviation checks before recommending calorie and macro adjustments. Users can apply recommendations, adjust trajectory, or keep current targets.
- **Macronutrient Analytics & Statistics Hub Integration (`StatisticsHubScreen`, `MacroSectionCard`, `MacroStatisticsScreen`, `MacroHistoryStackedBarChart`, `MacroAnalyticsDataAdapter`):** Added a new dedicated "Ernährung" (Nutrition) analytics section to `StatisticsHubScreen` positioned below Training. Features the interactive `MacroSectionCard` preview that synchronizes reactively with the hub's selected timeframe (7D, 1M, 3M, 6M, 1Y, MAX) and anchor date, displaying downsampled stacked mini-bars (Carbs, Fat, Protein proportional to daily calories) and period averages (`XXXX kcal >`). Tapping seamlessly preserves the active timeframe and anchor date into `MacroStatisticsScreen`, complete with timeframe filters, previous/next period navigation (`< Date Range >`) with interactive date picker, 4-metric summary header (Calories, Protein, Fat, Carbs), edge-to-edge stacked bar chart without redundant card nesting, dual-mode bar rendering (in-segment macro gram values for P/F/C $\ge 5$g, top calorie badges, and 2-letter weekday labels in 7D view; clean high-density bars in >7D view), horizontal caloric reference grid lines (0, 2000, 4000 kcal), interactive segment selection, and chronological daily breakdown list.
- **Dynamic Check-in Cadence & Goal-Anchored Weekly Reviews (`RecommendationScheduler`, `AdaptiveNutritionRecommendationService`, `WeeklyGoalReviewOrchestrator`, `LegacyGoalMigration`):** Replaced the rigid Monday-to-Sunday check-in cycle with dynamic check-in weekday cadence anchored to the active goal's start date (`goal.startDate.weekday`, e.g. every Thursday for goals started on a Thursday; fallback: Monday). Decoupled manual in-week recalculations ("Neu berechnen & Anwenden") so applying target updates immediately adjusts daily calorie and macro goals without disrupting or shifting the established weekly check-in day. Preserved legacy Monday cadence for migrated historical goals.
- **Rolling 7-Day Intake vs. Target Comparison Chart (`NutritionHubScreen`, `NutritionRecommendationCard`, `WeeklyIntakeVsTargetChart`, `MacroAnalyticsDataAdapter`):** Integrated the interactive 7-day intake comparison chart into the adaptive recommendation card inside the Nutrition Hub. Displays 7 vertical stacked bars for actual intake over the last 7 rolling days leading up to today (`fetchRecentDays(days: 7)`), complete with total calories badge and macro tags (`P`, `F`, `C`), separated by a clear visual divider gap from an 8th target bar ("Ziel") showing recommended target calories and macros. Streamlined the card layout by moving redundant target metric tiles into the expandable explanation drawer ("Why this recommendation?").
- **Goal Notifications (`GoalNotificationSettingsScreen`, `LocalNotificationService`):** Added configurable notifications for weekly goal check-ins and target date reminders (approaching / reached) with dedicated notification channels.

- **Goal Adjustment & Retire Flow Polish (`GoalAdjustmentSheet`, `GoalDetailScreen`, `glass_bottom_menu`):** Transformed the Goal Adjustment flow into the app's standard `showGlassBottomMenu` liquid glass sheet outside weekly reviews. Removed the duplicate "Adjust Goal" heading, stripped all emojis and leading icons from the adjustment rows, replaced the segmented control with the reusable Apple HIG `AppSegmentedControl`, gave Target Weight and Planned Weekly Rate custom liquid glass number input sheets (`showGlassNumberInput`), and added glass confirmation dialogues before updating plans (`showGlassConfirmation`) and retiring active goals (`showDeleteConfirmation`).
- **Goal Tracking UI Polish & ValueSummaryCards (`GoalProgressHeroCard`, `GoalDetailScreen`, `MeasurementChartWidget`, `GlassProgressBar`):** Streamlined the active goal hero card by removing redundant icon containers, "Drives nutrition" pills, and duplicate goal titles, replacing the custom stats row with clean `ValueSummaryCard` widgets (top prominent Current card with delta subtitle, 2-column Baseline and Target grid underneath, fixed full-width expansion). In `GoalDetailScreen`, aligned all card margins and spacing strictly to 8px (`DesignConstants.spacingS`), removed the nested container box and outer borders around personal motivation in favor of a clean inline quote row, replaced the 3 custom summary cards with `ValueSummaryCard`, replaced the bottom progress card with a sleek `GlassProgressBar` matching the active badge's green (`Colors.green`) with exact 19px squircle rounding (`DesignConstants.borderRadiusL`), consistent theme-brightness text rendering (crisp black text in light mode, crisp white text in dark mode across filled and unfilled tracks), and concise localized "Progress" label. Enhanced `MeasurementChartWidget` to span the entire goal timeframe with date-based x-axis and an interactive linear dashed trajectory line connecting starting weight to target weight.
- **Nutrition Hub & Goal Dashboard Integration (`NutritionHubScreen`, `ActiveGoalDashboardWidget`, `GoalDetailScreen`):** Replaced the single `GoalProgressHeroCard` in the Nutrition Hub with the complete active goal dashboard (`ActiveGoalDashboardWidget`). This mirrors the comprehensive `GoalDetailScreen` view within the Nutrition Hub—featuring the full header card with motivational quote, active status, and started date; the 3-column `ValueSummaryCard` metrics (Baseline, Current, Target); the animated `GlassProgressBar` with remaining distance, percentage, and weekly rate; and the full-width edge-to-edge trajectory chart with historical data points—omitting only the adjust and retire action buttons. In addition, refactored `GoalDetailScreen` to reuse `ActiveGoalDashboardWidget` to eliminate code duplication and ensure identical presentation across both screens.
- **Adaptive Nutrition Recommendation Card & Pacing (`NutritionHubScreen`, `NutritionRecommendationCard`, `AdaptiveNutritionRecommendationService`, `GoalRepositoryImpl`):** Restored the dedicated section header (`l10n.adaptiveRecommendationCardTitle`) above the adaptive recommendation card in the Nutrition Hub and removed the redundant "My Goals" navigation button. In `NutritionRecommendationCard`, swapped the layout so that Recommended Targets (`_MacroTargetGrid`) is prominently displayed on top, while Estimated Maintenance (`_MaintenanceHero`) is positioned underneath. Connected the adaptive recommendation engine to dynamically compute the required weekly rate from current body weight and remaining weeks to the active goal's target date upon 7-day evaluations. Applying a nutrition recommendation now changes only daily calorie and macro targets; goal pace, date, and target remain explicit trajectory decisions.
- **Nutrition Hub & Recommendation Card (`NutritionHubScreen`):** Restored the 1:1 `NutritionRecommendationCard` alongside the active goal dashboard in the Nutrition Hub, removed the redundant duplicate "Nutrition" body header, and removed obsolete daily operating target cards.
- **Interactive Goal Creation Flow (`CreateGoalFlow`):** Redesigned the goal creation flow into 6 focused steps centered on body weight as the sole adaptive engine metric. Features an interactive bidirectional pace & target date planner where adjusting target date recalculates weekly rate live and adjusting weekly rate recalculates projected duration live, complete with physiological pace indicators, calorie impact estimates, and automatic baseline detection.
- **Goals Screen Navigation & Parallel Access (`GoalsScreen`):** Kept existing daily macro settings fully functional with a top banner seamlessly linking to long-term goal management, ensuring zero disruption to existing workflows.

### Changed
- **Training Plan Overview Direct Background Layout (`ManualPlanScreen`, `PlanOverviewCard`, `PlanScheduleOverviewGrid`):** Removed the outer nested `SummaryCard` and redundant "Planübersicht" header text and pencil icon from the training plan screen. The 2-column schedule tiles are now rendered directly on the screen background using standard primary surface styling, and tapping any tile directly opens the plan editor.
- **Onboarding Design Consistency & Visual Alignment (`OnboardingScreen`, `UnitSystemSlide`, `RegionSelectionSlide`, `OnboardingInfoBox`, `GoalPresetStep`, `GoalTargetStep`, `GoalMotivationStep`):** Standardized top spacing to a uniform 48px across all onboarding and embedded goal flow slides. Left-aligned all slides for clean typographic hierarchy while keeping `WelcomeSlide` centered. Replaced custom selection indicators with the consistent right-aligned `LucideIcons.circle_dot` / `LucideIcons.circle` radio pattern. Unified prominent numeric displays to `textTheme.displaySmall` (36pt) with bold weight and tabular figures. Created a reusable, standardized `OnboardingInfoBox` callout component with `LucideIcons.info` and rounded container styling, replacing disparate inline containers. Refined text input fields and dropdowns with transparent background styling, consistent corner radii, and contextual prefix icons (`LucideIcons.activity`, `LucideIcons.timer`). Applied compact visual density and consistent border styling to quick delta chips in `GoalTargetStep` to match `GoalMotivationStep`. Replaced hardcoded spacing constants with design tokens (`DesignConstants.spacingXL`, `spacingM`, `spacingS`).
- **Workout Hub & Training Plan UI Refinements (`WorkoutHubScreen`, `ManualPlanScreen`, `WorkoutPlanHeroCard`, `ColdStartEmptyState`):** Replaced the bulky `SummaryCard` for empty routines in the Workout Hub with a clean, compact, centered empty state container with subtle translucent styling and a muted swatch book icon, taking minimal vertical space and visually distinguishing itself from real routine items. Redesigned the Training Plan screen empty state to use the app-standard `ColdStartEmptyState` featuring a curved arrow pointing down to a `GlassFab` ("Create plan") button; the floating action button and arrow are only shown when no training plans exist, while screens with existing plans retain the clean top app bar creation action. Streamlined `WorkoutPlanHeroCard`'s eyebrow header by eliminating the neon green accent color and forced uppercase casing in favor of the app's standard muted `onSurfaceVariant` label styling.
- **Training-plan action and day-card cleanup:** The plan overflow action now uses the shared platform-adaptive menu, version history no longer repeats revision numbers in colored badges, and the selected-day card uses a quieter hierarchy without redundant date/status prose or a divider.
- **v1.5 Release Preparation:** Completed and synchronized localized App Store release notes for all supported storefronts, refreshed TestFlight and App Review guidance, tightened store-description claims, and removed stale localization entries so the shipped locale catalogs stay aligned.
- **Manual Plan Experience:** Redesigned manual plans around a Diary-style seven-day calendar rail, a matching calendar button that opens the app-wide week picker, and one focused session card with an exercise-and-set preview. The plan and editor screens now use the standard frosted app bar, active plans no longer carry a redundant status accent, inactive plans are called out in red, and the Diary entry follows the same ListTile typography and spacing as its surrounding summary cards while retaining direct workout start. The editor uses explanatory plan-type cards, direct add/remove/reorder controls for 1–14 day sequences, routine and set metadata, and an always-reachable save action; revision history is presented as a readable visual timeline. Week navigation still begins at the plan's first activation instead of exposing empty pre-plan history.
- **Macro Analytics UI & Interaction Polish (`MacroSectionCard`, `MacroStatisticsScreen`, `MacroHistoryStackedBarChart`):** Aligned `MacroSectionCard` in the Statistics Hub with standard analytics cards by incorporating `AnalyticsCardBase.buildHeaderWithChevron` with timeframe pill and trailing chevron, while removing redundant month subtitles and bottom chevron icons. In `MacroStatisticsScreen`, removed redundant date range text below the 4-metric average row, extended the stacked bar chart to start from the left edge of the screen, replaced the layout-shifting inspection banner with a zero-layout-shift press-and-hold / drag-to-scrub interaction: while the user presses and holds any bar (or drags across bars), the hovered bar highlights, the 4-metric header instantly updates to show that day's exact calories, protein, fat, and carbs (label prefix switches from "Ø" to plain to indicate day-mode), and a small floating date chip appears directly above the active bar inside the chart without affecting surrounding content. Releasing the finger resets to period averages. Standardized the "Tagesverlauf" section header with `AppSectionHeader`, removed the phantom top gap before the list, and reformatted daily entries into clean 2-line cards featuring dates and colorful macro badges with the duplicate calorie count removed.
- **Responsive Adaptive TDEE & Trajectory Control:** Shortened the adaptive nutrition signal window to 14 days, increased weekly TDEE process variance to $60^2$, replaced the phase-dependent 3,000→7,700 kcal/kg observation ramp with a fixed 7,700 kcal/kg coefficient plus temporary phase-transition uncertainty, and added a separately bounded rate controller for persistent goal-trajectory error. Existing applied targets remain unchanged until explicitly applied, legacy estimator state is migrated without double-updating the current week, and the recommendation UI and algorithm information sheet now distinguish maintenance estimation from trajectory correction.
- **Flat Weekly Review & One-Click Plan Choice:** Simplified the weekly review into a flatter information hierarchy without a separate card around every metric, embedded the complete target/date/rate editor directly in the screen, and added a highlighted one-click recommendation that keeps a realistic pace and calculates the corresponding target date.
- **Goal Creation Wizard & Ruler Pickers (`CreateGoalFlow`, `AppRulerPicker`, `GoalAdjustmentSheet`, `glass_bottom_menu`):** Replaced default Flutter sliders and raw text fields across the goal creation flow with the tactile horizontal ruler picker (`AppRulerPicker.weight`, `AppRulerPicker.rate`) with haptics, edge fading, center accent marker, and tabular figures for baseline weight, target weight, and goal adjustment sheets. Replaced custom goal direction options with the Apple HIG `AppSegmentedControl`. Redesigned Step 3 ("Plan Pace & Target Date"): relocated the live trajectory projection card prominently to the top as a primary hero element, replaced loose chips with platform-adaptive liquid glass dropdown menus for Pace (Gentle, Moderate, Athletic, Aggressive, and Custom Pace revealing the ruler slider) and Duration (8, 12, 16, 24 weeks, and Custom Target Date with adaptive calendar picker), eliminating empty screen space and providing a cohesive, polished look.
- **5-Step Goal Creation Streamlining & Unified Design (`CreateGoalFlow`):** Refactored the goal creation wizard into a streamlined 5-step flow. Step 1 (Start Date & Baseline) now lives in a single, unified `SummaryCard` matching Train Libre styling without disjointed list tiles or status boxes. Removed the redundant "Open goal" mode and empty placeholder screen for bodyweight changes, combining Target Weight and Pace into an interactive real-time planner (Step 2). For Maintain Weight / Recomposition goals, Step 2 provides an informational maintenance corridor ($\pm 1.0$ kg) and ongoing or preset duration choices at neutral energy balance. Separated personal motivation into a quiet, dedicated screen (Step 3) and isolated final parameter verification and the "Drives nutrition" switch on a standalone Review & Activate screen (Step 4).
- **Goal Creation Flow Visual Polish (`CreateGoalFlow`, `ValueSummaryCard`):** Replaced the plain text planned change box at the bottom of Step 2 (Target Weight) with the app's standard 3-column `ValueSummaryCard` row (Start/Baseline, Planned Change delta, and Target). In Step 3 (Plan Pace & Target Date), removed the calendar icon next to Duration, cleaned pace dropdown labels to avoid duplicate rate text, and wrapped the daily calorie delta row in `Expanded` with concise wording to eliminate 1-line layout overflow. In Step 4 (Motivation), removed all emojis and icons from suggestion chips, expanding the list into a compact 10-item motivation catalog with active selection styling that wraps across lines.
- **Domain-Specific Nutrition Goal Access & Safe Adjustment (`MyGoalsScreen`, `CreateGoalFlow`):** Switched `MyGoalsScreen` to use `getActiveNutritionGoal()` explicitly, disabled goal trajectory adjustment when neither an active progress measurement nor baseline snapshot is present, and ensured the target weight ruler picker only renders when a valid baseline or target exists.

- **Goal Creation Flow Tactile Baseline Picker & UI Refinements (`CreateGoalFlow`, `PlatformAdaptiveSwitchListTile`, `glass_bottom_menu`):** Upgraded Step 2 (Start Date & Baseline Weight) to feature the tactile `AppRulerPicker.weight` with prominent numeric weight display and toggleable manual input, matching Step 3. When an existing baseline weight measurement is present, the ruler picker is neatly collapsed by default and only reveals itself upon tapping the edit action ("Ändern" / pencil). Made quick adjustment chips in Step 3 scroll edge-to-edge across the screen width. In Step 5 (Review & Activate), replaced the standard AlertDialog with the reusable `showGlassConfirmation` liquid glass bottom sheet for replacing existing active goals, adopted the platform-adaptive `PlatformAdaptiveSwitchListTile` for the "Drives nutrition" toggle, and removed icons from planned rate and target date rows in the review card and maintain corridor.

### Fixed
- **Backup Restore Foreign Key Constraint & Database Lock Fix (`BackupManager`, `DriftDatabase`, `ICloudSyncService`):** Fixed backup restoration failures caused by `SqliteException(787): FOREIGN KEY constraint failed` and `SqliteException(5): database is locked`. Disabled foreign key constraints during restore transactions so tables and cross-table references (such as `user_goals.baseline_measurement_id`, aliased exercises, and nutrition logs) can be wiped, repopulated, and sanitized safely before foreign keys are re-enabled. Added raw `measurements` table export and restore to preserve UUIDs and exact timestamps across backups. Added automatic reconciliation and sanitization for orphaned goal baselines, predecessor goals, events, reviews, and logs. Configured a 30-second SQLite `busy_timeout` across database connections and added retry backoff for locked transactions to avoid clashes with background catalog imports. In addition, resolved an iCloud storage plugin crash (`type 'Null' is not a subtype of type 'bool'`) by sanitizing nullable boolean properties returned by iOS native metadata.
- **Training Plan Version History Sheet Card Width (`ManualPlanScreen`, `_PlanRevisionTile`):** Fixed inconsistent card widths across training plan revision items in the "Versionen" sheet. All revision cards now stretch to the maximum available sheet width uniformly regardless of varying chip/text lengths.
- **ProfileService Lifecycle & Disposal Fix (`ProfileService`, `main.dart`, `profile_slide`):** Fixed an `AssertionError: A ProfileService was used after being disposed` crash when building `NameSlide` during onboarding. `ProfileService` is a singleton service that was erroneously provided using `ChangeNotifierProvider(create: ...)`, causing Provider to dispose the singleton on route replacement or rebuild and leaving `ProfileService` permanently dead. Initialized `ProfileService` alongside other persistent services before `runApp()`, provided it using `ChangeNotifierProvider.value(value: profileService)` to prevent framework disposal, added disposal state tracking with safe factory re-instantiation, and guarded listener notifications.
- **Onboarding body-fat ruler scale (`OnboardingMeasurementsStep`):** Corrected the ruler to use the same 0.1-unit tick geometry as the weight ruler, preventing overlapping labels while retaining whole-percent labels.
- **Onboarding height ruler direction (`profile_slide`):** Flipped the vertical drag mapping so the ruler follows the user's swipe direction.
- **Onboarding gender default (`OnboardingScreen`):** Kept the initially displayed “Male” selection synchronized with the submitted profile state, preventing a false required-field validation error until the user reselected it.
- **Onboarding Goal Flow Layout & Spacing Alignment (`OnboardingScreen`, `GoalPresetStep`, `GoalTargetStep`, `GoalPaceTimelineStep`, `GoalMotivationStep`):** Fixed missing and inconsistent horizontal margins and top offsets across the integrated onboarding goal steps. Passed standard `DesignConstants.spacingXL` side margins and uniform top offsets matching existing onboarding slides, aligning title, subtitle, cards, suggestion chips, and inputs consistently with the bottom navigation actions.
- **Deleted Planned Workout Status:** Deleting a workout that was started from a manual plan now atomically removes its linked plan occurrence, reopening the scheduled day instead of leaving it stuck on “In progress”. Calendar loading also repairs orphaned occurrences created by earlier builds.
- **Beverage Calorie Aggregation & Intake Chart Scaling Fix (`MacroAnalyticsDataAdapter`, `WeeklyIntakeVsTargetChart`, `FluidEntry`):** Fixed a critical calculation bug where `FluidEntry.kcal` (which already represents total calories for the consumed beverage) was erroneously multiplied a second time by `quantityInMl / 100.0`. For large fluid logs (e.g. 4850 ml with ~78 kcal), this added an artificial 3,781+ kcal multiplier on top of actual intake (causing 4,277 kcal to explode to 8,058 kcal). In addition, wrapped the top calorie badge and stacked macro segment text (`P`, `F`, `C`) in `FittedBox.scaleDown` within `WeeklyIntakeVsTargetChart` to prevent number clipping on narrow daily columns.
- **PageStorage Key Collision Fix (`CreateGoalFlow`, `MyGoalsScreen`, `WeeklyGoalReviewScreen`, `NutritionRecommendationCard`):** Resolved runtime `type 'double' is not a subtype of type 'bool?' in type cast` exception by assigning explicit, unique `PageStorageKey`s across all expansion tiles and wrapping the goal creation flow in an isolated `PageStorage` bucket, preventing scroll offset and expansion state collision across routes.
- **Strict Weight Validation & Historical Measurement Fallback (`RecommendationInputAdapter`, `DatabaseHelper`, `AdaptiveNutritionRecommendationService`):** Eliminated all artificial 75 kg fallbacks in recommendation, onboarding, and goal tracking. The adaptive recommendation engine now strictly requires a real measured weight or immutable baseline snapshot and safely rejects non-positive values. When no measurement falls into the 14-day adaptive window, the adapter loads the latest real weight observation on or before window end or uses the active goal's immutable baseline snapshot without inventing numbers, keeping observation counts and trend slope honest.
- **Overall-First Weekly Goal Assessment:** Weekly reviews now judge the complete goal trajectory against the expected value for today, show the recent 7-day trend separately as momentum, calculate the rate required from now and a projected target date, and distinguish nutrition adjustments from goal/date/rate changes. This fixes mixed cases that previously reported "too fast" despite being substantially behind over the full goal period, and prevents backdated goals from fabricating historical weekly reviews.
- **Goal Event SQLite Constraint Fix (`GoalEvents`, `DriftDatabase`):** Added the `occurred_at` timestamp column with automatic `currentDateAndTime` default in the Drift schema and schema reconciliation migration, preventing SQLite NOT NULL constraint failures on goal creation.
- **Nutrition DevLab Test Data Seeder SQLite Unique Constraint Fix (`NutritionTestDataSeeder`):** Resolved `SqliteException(2067): UNIQUE constraint failed: products.barcode` by replacing the `insertOnConflictUpdate` call (which only caught primary key conflicts on `local_id`) with safe lookup-by-barcode/id before update or insert, and ensured test products are properly deleted on scenario reset.
- **Local Notification Banners & iOS Foreground Delegate Setup (`AppDelegate.swift`, `LocalNotificationService`, `NotificationTestView`):** Registered `UNUserNotificationCenter.current().delegate` in `AppDelegate.didFinishLaunchingWithOptions` to enable iOS foreground banner delivery and notification action handling. Added explicit `presentBanner: true`, `presentList: true`, `presentSound: true`, and `presentBadge: true` across all channels, OS-level `zonedSchedule` delays for 5-second background testing, detailed error logging, preference bypasses (`ignorePreferences: true`), and an inline permission status card with one-tap permission request in the Developer Lab.
- **Automatic Adaptive Recommendation Recalculation & Application on Weekly Review Completion (`WeeklyGoalReviewScreen`, `AdaptiveNutritionRecommendationService`):** Completing a weekly goal review—whether accepting recommended targets, keeping previous targets, or saving a trajectory adjustment—now automatically triggers an immediate recommendation recalculation (`recalculateAndApply`) and updates active daily macro targets seamlessly without requiring manual triggers or page refreshes.
- **Standardized Screen App Bars (`GlobalAppBar`):** Replaced Flutter's generic `AppBar` with the project's standardized, frosted-glass `GlobalAppBar` across `MyGoalsScreen`, `GoalDetailScreen`, and `WeeklyGoalReviewScreen`.
- **Goal Creation Motivation Field Interactivity (`CreateGoalFlow`):** Resolved text input focus and touch-target issues in the motivation step by assigning persistent state keys, a dedicated `FocusNode`, and adding one-tap motivation suggestion chips.
- **Adaptive Service Database Dependency (`AdaptiveNutritionRecommendationService`):** Passed the shared database instance to `GoalRepositoryImpl` within the recommendation service constructor, preventing background attempts to open a secondary database connection during testing and operations.
- **AppButton Accessibility Assertion (`AppButton`):** Removed an erroneous assertion requiring `tooltip` or `semanticsLabel` on buttons with a required visible text `label` and built-in semantic fallback, fixing a red-screen assertion crash in debug mode across the app.
...
### Changed
- **Fat Recommendation Raised off its Floor (`AdaptiveNutritionRecommendationEngine`, #252):** The adaptive recommendation set fat to a flat floor of 0.60 g/kg body weight regardless of goal or calorie budget and gave every remaining calorie to carbohydrates — the very bottom of the evidence-based cutting range (0.5–1.0 g/kg) and below the maintenance range (0.8–1.2 g/kg). Fat is now targeted per kilogram like protein already was: **0.9 g/kg while cutting, 1.0 g/kg at maintenance and on a bulk**, with carbohydrates taking the remainder. For an 80 kg user at 2,600 kcal this moves the recommendation from 48 g to 80 g of fat.
  - **Graceful Degradation:** On a calorie budget too small to carry both targets, fat now gives way towards the 0.60 g/kg floor before carbohydrates are emptied, instead of the distribution collapsing straight into the constrained fallback. The existing `macro_distribution_constrained` warning still fires when nothing fits.
  - **Algorithm Version Bump (`tdee_adaptive_recommendation_1_1_bayesian_recursive`):** Stored recommendations from the previous macro model are treated as stale by `AdaptiveRecommendationSnapshot.isFreshFor` and recomputed on the next due check, so existing users receive the corrected distribution rather than keeping an old one alongside new ones.
  - **Public Documentation (`docs/adaptive-nutrition/`):** Added a "From calories to macronutrients" section documenting the protein and fat targets per kilogram, the carbohydrate remainder, and the fat floor, translated into all five supported languages.
  - **Technical Documentation (`documentation/features/macro_distribution.md`):** New reference covering the second stage of the nutrition recommendation, which had no documentation of its own: input resolution order, the per-kilogram anchors and their rationale, the fat floor, the two-stage degradation path under a constrained budget, worked examples generated by the engine, and the versioning rule that any change to these values must bump `algorithmVersion`. Linked from the capabilities overview, the documentation index, and a new handover section at the end of the TDEE estimator's Kalman chapter.
- **Telemetry Consent Decoupled from the Mandatory Consent (`TelemetryConsentSheet`, `TelemetryConsentPrompt`):** The optional telemetry question was a second checkbox on the initial consent screen, directly below the privacy policy consent the user has to give to use the app at all. It is now asked on its own, in a bottom sheet shown immediately after the mandatory consent is granted — an optional consent bundled into a form that must be accepted is hard to argue was freely given. The sheet presents the accept and the decline as two equally sized buttons in one row with nothing pre-selected, names what is and is not collected, and states that data is never sold. Telemetry is written to the off state before the sheet opens, so a force-quit mid-question cannot leave it enabled.
  - **A Single Follow-Up, Never a Third Ask:** If the question is declined during onboarding, the same sheet returns exactly once — gated on 14 calendar days *and* 5 app launches, and appended to the existing startup-prompt chain so it can never stack with the app tour, release notes, or review prompt. Whatever the second answer is, including a dismissal, it is never shown again. Installations predating the prompt are anchored on their first launch after updating and serve the same waiting period. The state lives in `SharedPreferences` and therefore survives a backup restore. A decline is never tracked — without consent nothing may reach PostHog, an "opted out" event included.
  - **Telemetry Settings Localized (`SettingsScreen`):** The telemetry switch and the data deletion row carried hardcoded German strings and showed German text to English, French, Italian and Japanese users. Both now go through the ARB files in all five languages.
  - **Refined Copywriting & Pseudonymisation Clarity (`TelemetryConsentSheet`, `SettingsScreen`, `LegalScreen`):** Aligned wording across all five supported languages with technical and GDPR standards (Art. 4, 6, 7 & 9 GDPR): replaced "anonymous" with "pseudonymised" to accurately reflect random device/session UUIDs, explicitly excluded workout, diary, nutrition and health data, removed emotional pressure ("only way"), and clarified that disabling the switch halts collection while past data can be deleted separately via the deletion action in Settings.

### Fixed
- **Caffeine Missing from Diary, Supplement Screen and Progress Bar (`Supplement.isCaffeine`, `ensureStandardSupplements`):** Caffeine logged through fluids and food appeared in the fluid dialog and nutrition detail but never in the supplement screen or the caffeine progress bar. The caffeine row was identified by comparing its name against the literal string `'caffeine'` in eight separate places, which fails for every localized name (Koffein, Caféine, Caffeina, カフェイン) on rows seeded before the `code` column existed — so no supplement log was ever written. Identification now runs through a single `Supplement.isCaffeine` helper that matches the canonical code first and falls back to the localized names only for rows without one.
  - **Self-Healing Seeding:** `ensureStandardSupplements` seeded caffeine and creatine only when the supplements table was completely empty, so anyone who had created a single supplement of their own never received the caffeine row. It now checks per code, back-fills the canonical code onto a legacy row instead of duplicating it (preserving the user's own dose, goal and limit), and heals existing installations on the next cold start.
- **Morph Animation Drew Cards Twice (`CardMorphRoute`, `MorphSourceScope`):** Container transforms drew the flying copy of a card while the original stayed in place underneath, so the same card was visible twice at two sizes for most of the collapse — most obviously on the way back, once the container's background had faded out. All 49 morph call sites now hand the route both a source copy and a visibility handover, and the original is taken off screen for the flight and handed back at the very end, while the copy still covers it exactly.
  - **Source Copy Built Once (`CardMorphRoute`):** A `sourceBuilder` closes over the screen it came from, and screens holding their content in a `FutureBuilder` replace that whole subtree with a spinner when they reload — which happens the moment the pop starts, not when the collapse ends. Rebuilding the copy at that point threw "Looking up a deactivated widget's ancestor is unsafe" on every frame of the collapse in the Nutrition Hub. The copy is now built once, on the frame after the push, and reused for the whole flight.
  - **Nutrition Hub Reload No Longer Blanks the Screen (`NutritionHubScreen`):** The hub showed its loading spinner in place of all content on every reload, including the one triggered while a morph back into one of its cards was still in flight. The spinner is now limited to the first load; reloads keep the data already on screen.
  - **Meal Card Flashed its Ingredient List (`MealEntryCard`):** An expanded meal card briefly showed its full sub-item list in the frames where the flying copy — always a fresh, collapsed card — overlapped it at the end of the collapse. The card now folds itself away for the duration of the flight, while it is off screen, and stays folded.
- **Performance Log Reset Left Startup Timings Behind (`PerformanceDiagnosticsScreen`):** The delete button reset the jank recorder but not `StartupTrace`, so the cold start and resume runs survived clearing the log and remained in SharedPreferences (`perf_startup_trace_v1`). Both sources are now cleared together.
...
### Changed
- **Goal Creation UI Polish (`CreateGoalFlow`):** Restyled goal-type selection to match the training-plan selector’s animated cards while retaining a readable single-column layout, separated start date and baseline weight into distinct cards, split target-weight selection from pace/timeline planning into two focused steps, and gave the onboarding progress track a visible primary-tinted background in dark mode.
- **Workout Plan UI Polish (`DiaryScreen`, `WorkoutHubScreen`, `ManualPlanScreen`):** The Diary now shows either the active plan card or the completed-workout summary instead of duplicating both. Workout Hub start buttons no longer repeat a play icon, “New Routine” uses a folder-plus symbol, and the new training-plan cards now open plan and live-workout screens with the shared fluid card-morph transition.
...
- **Android Home Screen Widgets:** The iOS widget suite ported to Android with Jetpack Glance, on top of the snapshot pipeline that already existed: `HomeWidgetSyncService` writes one JSON payload over the `trainlibre.widgets/home_screen` MethodChannel and the widgets only render it, so no Dart logic is duplicated per platform. Opening `lib/features/home_widgets/` needed a single line of change — the `Platform.isIOS` gate in `HomeWidgetChannel.isSupported`.
  ...
  - **Last Workout (4x2 / 4x4):** Duration, volume and sets plus the muscle heatmap the app renders when a session is finished. Switches to a stacked layout that gives the map the lower half once the widget is tall enough.
  - **Muscle Readiness (4x2):** The three readiness pills with their counts, shares and state colours.
  - **Steps (4x2):** The seven-day bar chart with its goal line, goal-met badges and locale-aware weekday initials.
  - **Body Measurements (4x2, configurable):** Sparkline and delta chip over a selectable metric and timeframe. The metric list comes from the snapshot, so it offers exactly what the user actually records.
  - **Quick Actions (2x2 / 4x2, configurable):** Two or four action tiles drawn from the same seven actions as the in-app speed dial.
  - Charts are drawn to bitmaps with `android.graphics.Canvas`: Glance compiles to RemoteViews, which has no canvas, no custom shapes, no fractional weights and no text shadows, so a composed version would have been a different design rather than the same one. Each configurable widget carries a Compose configuration Activity, declared `reconfigurable` so the launcher can reopen it on a widget placed weeks ago.
  - Both generated inputs have a generator rather than being hand-maintained: `script/android_widgets/xcstrings_to_android.py` converts the iOS string catalogue into `res/values*/widget_strings.xml`, and `script/android_widgets/lucide_to_vector.py` produces the icon drawables from the Lucide set the app itself draws with.
- **Android Quick Settings Tiles:** All seven quick actions as `TileService` tiles — the counterpart to the iOS Control Center controls. Each opens the same `trainlibre://action/<key>` deep link the widget tiles and the speed dial use, so all three surfaces end in one handler.
- **Android Live Update for Workouts:** The workout Live Activity's counterpart, as an ongoing notification that Android 16 promotes to a Live Update with a status bar chip (`setRequestPromotedOngoing`, `POST_PROMOTED_NOTIFICATIONS`); below that it stays an ordinary ongoing notification with the same content and buttons. It answers the same `trainlibre.workout/live_activity` channel ActivityKit does, so `WorkoutLiveActivityService` needed no platform branch beyond its support check. The rest countdown runs in the notification's own chronometer instead of being pushed every second, and the buttons reach a `BroadcastReceiver` that queues commands for `consumePendingCommands`, so they still work with the app process gone. Deliberately not a foreground service: a posted notification already outlives the process, and a service would have cost a permission the app does not otherwise need.

### Changed
- **Android Rest Timer Sound Stays With `LocalNotificationService`:** `LiveWorkoutViewModel._usesNativeRestSound` is now gated to iOS explicitly. It was derived from "a live activity is running", which was always false on Android; with the Android live update in place that would have silently rerouted the rest sound to a native scheduler that does not exist there, and building one would have required an exact-alarm permission.
- **Widget Refresh Is Awaited Before the Channel Replies:** `HomeWidgetBridge` used to answer the app as soon as the snapshot was on disk and redraw the widgets afterwards. The app's most important write is the one it makes while being backgrounded, and Android freezes a backgrounded process before a fire-and-forget coroutine reaches `updateAll` — so that update was lost at exactly the moment it mattered.
...
### Added
- **"What's New" Sheet After an Update:** On the first launch after an update the app now shows a bottom sheet with the highlights of the versions the user has not seen yet, and the same list is reachable at any time from *About Train Libre → What's New*. The texts are authored per store locale in `metadata/whats_new/<locale>.md` and compiled into `lib/features/whats_new/domain/whats_new_content.g.dart` by `script/build_whats_new.py`, which also pushes them into the `## Release Notes` section of `metadata/app_store/<locale>.md` so the in-app and App Store wording cannot drift apart. Which release notes are due is decided by comparing the marketing version from `PackageInfo` (build number ignored, so a new CI build does not re-trigger the sheet) against `whats_new_last_seen_version` in `SharedPreferences`; a skipped update shows every release in between, notes for a version newer than the running build stay hidden, fresh installs are seeded silently so onboarding is not followed by a third greeting, and the stored version is only written once the sheet has been dismissed. `script/deploy_release.sh` now runs `build_whats_new.py --check` before it builds anything and asks for confirmation if the notes for the version in `pubspec.yaml` are missing, incomplete or out of sync with the store metadata.
...
- **No Catalog Download Was Offered After Restoring a Backup:** Restoring a JSON backup onto a fresh install left the app convinced it already had the wger exercise and Open Food Facts catalogs while both tables were empty, so the download prompt never appeared and the catalog screens stayed blank. Catalog installation is tracked purely in `SharedPreferences`, and `BackupManager` exported *every* preference key and rewrote all of them verbatim after `prefs.clear()` on import — but the JSON payload carries no catalog rows, only user data. The restored device therefore inherited the source device's `installed_training_version`, `installed_off_version_*`, `is_exercise_catalog_initialized` and `last_db_sync_app_version`. Everything downstream trusted those keys: `isOffDatabaseInitialized()` / `isExerciseCatalogInitialized()` read nothing but preferences, and every screen that gates the prompt on them (food explorer, scanner, add-food, exercise catalog, AI capture) saw "ready"; the empty-table self-heal inside `checkForBasisDataUpdate` was itself gated on `!isExerciseInitialized`, so a restored `true` flag skipped the very check written to catch this; and a matching `last_db_sync_app_version` made the base-food and category sync return early. Fixed on both sides. A new `CatalogStatePrefs` (`lib/core/infrastructure/catalog_state_prefs.dart`) enumerates the keys that describe what is on *this* disk; backups no longer export them, the importer skips any that older backups still contain, and the device's own values are carried across `prefs.clear()` so restoring onto a device that *does* have the catalogs still re-downloads nothing. Independently, `BasisDataManager` no longer takes the preferences at face value: both `isInitialized` checks confirm the rows exist (memoized once verified, so the hot screen paths do not re-query), the exercise health check runs unconditionally, base foods and categories are verified the same way, and an OFF version claim that no rows back up is cleared so the prompt is offered instead of a phantom "up to date". Covered by regression tests for the export filter, both import directions, and the two presence checks.
- **iCloud Restore Left the App Reading the Pre-Restore Database:** `downloadAndRestore` renamed the downloaded snapshot over `app_hybrid.sqlite` while the live Drift connection was still open, leaving that connection bound to the now-unlinked old inode; the caller closed it only afterwards, via `DatabaseHelper.driftDb?.close()`, which never cleared the static — so `dbInstance` kept handing out the closed database, and the `UnitService.reload()` and `_loadAdaptiveGoalSettings()` calls immediately following the restore ran against it. Stale `-wal`/`-shm` sidecars belonging to the previous database were also left beside the restored file, where SQLite would attempt to recover from them. The restore now closes and clears the connection itself before the swap (new `DatabaseHelper.closeAndResetDriftDb()`), deletes the sidecars, then extracts the bundled preferences and drops `BasisDataManager`'s catalog-presence memo; the onboarding caller no longer closes anything on its own.
...
### Security
- **AI API Keys Were Included in iOS Device and iCloud Backups:** BYOK provider keys are stored in the Keychain via `FlutterSecureStorage` and correctly never appear in the app's own backups — neither the JSON export (`BackupManager` reads only `SharedPreferences`, which the keys never touch) nor the iCloud snapshot (`VACUUM INTO` of the Drift database plus `_bundleSharedPreferences`). Apple's backups were the leak: `const FlutterSecureStorage()` defaults to `KeychainAccessibility.unlocked` (`kSecAttrAccessibleWhenUnlocked`), which — unlike the `…ThisDeviceOnly` classes — is captured by iOS device and iCloud backups and restored onto whatever device the backup is applied to, so a user's OpenAI/Gemini/Anthropic key left the device with every backup. `AiService` now writes every item it owns (`ai_api_key_*`, `ai_selected_provider`, `ai_selected_model_*`, `ai_custom_base_url`, `ai_custom_model`, `ai_timeout_seconds`) with `KeychainAccessibility.unlocked_this_device`, exposed as `AiService.deviceOnlySecureStorage`; `LocalAppDataResetService` shares the same instance, since `kSecAttrAccessible` is part of the keychain lookup query and a mismatch would make "delete all local data" silently miss the items. Existing installs are migrated by `AiService.migrateSecureStorageToDeviceOnly()`, run once from `main()` before anything reads storage: for each key it reads with the legacy options, deletes, then rewrites device-only. The delete is required rather than a plain overwrite because `SecItemAdd` rejects a duplicate service/account pair while a new-options read cannot see the old item; the plugin's own delete path clears `accessibilityLevel` from the query, so it removes the backup-eligible item regardless. The done-flag is only set after every key succeeds, so a failure retries on the next launch instead of half-migrating. Android is unaffected: the AES key is wrapped by a hardware Keystore key that is never backed up, making any copied ciphertext undecryptable off-device.
