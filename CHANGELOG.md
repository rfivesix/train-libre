...
- **Leaving the Review Screen Asks Before Discarding:** Back now returns all the way to where the capture started, so a stray back swipe would otherwise throw away a finished analysis with nothing to undo it. Saving is unaffected — it leaves without a question.
