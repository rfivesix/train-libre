...
### Added
- **Delete Custom Exercises:** Users can now permanently delete their own custom exercises directly from the exercise detail screen. A trash icon appears in the AppBar for user-created exercises. Tapping it opens a confirmation bottom sheet explaining that the exercise will be removed from routines, and that existing workout log entries are preserved (the exercise name snapshot remains, only the DB reference is cleared). Deletion is transactional: all `set_logs` foreign key references are nulled, `routine_exercises` entries are removed, and the exercise and its translations are hard-deleted.
...
- **PostHog Native GeoIP & Environment Metadata:** Updated `TelemetryService.trackAppLaunched` and `PostHogTelemetryService` to transmit native PostHog location metadata (`$geoip_country_code` and `$locale`) and full standard device/environment metadata (`$app_build`, `$device_manufacturer`, `$device_model`, `$device_name`, `$device_type`, `$is_emulator`, `$is_sideloaded`, `$is_testflight`, `$os`, `$sent_at`, `$timezone`, `$lib`, `$lib_version`), ensuring `app_launched` contains the exact same standard device fields as SDK-captured events without GeoIP city profiling.
- **Complete App Localization (l10n):** Scanned codebase for unlocalized UI text and extracted all remaining hardcoded string literals (telemetry deletion dialog, local catalog update prompts, catalog titles, import feedback, and copy-to-clipboard notifications) into ARB files (`app_en.arb`, `app_de.arb`, `app_fr.arb`, `app_it.arb`, `app_ja.arb`). Applied `BodySlugMapper.localize()` to exercise category names in `ExerciseCatalogScreen` filter dropdown and exercise list tile subtitles, added `muscleLegs` / `muscleArms` localizations, and adjusted German `muscleAbs` to "Bauch".
- **Anonymized TDEE Telemetry (`recommendation_generated`):** Added zero-PII telemetry tracking for Bayesian TDEE recommendation runs, capturing data log counts (`weight_log_count`, `intake_logged_days`), window size (`window_days`), effective sample size (`effective_sample_size`), confidence level buckets (`confidence_score_bucket`), warning levels, and algorithm quality flags without transmitting body weight, calories, or personal data.
