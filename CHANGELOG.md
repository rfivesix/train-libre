...
### Changed
- **Manual Plan Calendar & Editor:** Reworked manual plans into a compact grouped agenda with an animated selected-day detail, clear status treatment, restrained iconography, explanatory guidance, and the shared statistics timeframe chip/picker. Week navigation now starts at the plan's first activation instead of allowing empty history before the plan existed, and the editor uses one clean grouped schedule rather than a stack of separate day cards.
