...
- **Performance and GC Optimizations:** Replaced multi-pass and chained iterable pipelines (`.where()`, `.map()`, `.toList()`, `.fold()`) with direct single-pass loops and O(1) set lookups across `SupplementsViewModel`, `BasisDataManager`, `ConsistencyTrackerScreen`, `AnalyticsCardBase` mini-bars, `SleepPipelineService`, `SleepDayRepository`, `nightly_metrics_calculator`, and `PulseAnalysisEngine`.
- **Accessibility & Semantics Enhancements:** Added accessible labels and semantics to `SetTypeChip` using localized set type strings, enhanced `TimeRangeFilter` navigation arrows with localized previous/next tooltips, and added semantic labels to the main speed dial action button and overlay close action.
