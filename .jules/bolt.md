...
## 2024-05-18 - [Optimizing batch asset imports by combining type filtering]
**Learning:** In Flutter batch processing loops (e.g. basis_data_manager parsing thousands of initial exercise records from SQLite assets), using chained operations like `.whereType<T>().toList()` and `.where((c) => c is! T).toList()` on the same underlying large isolate array payload processes the entire list twice and unnecessarily allocates separate intermediate Iterables.
**Action:** When splitting a dataset into distinct type-based lists within performance-sensitive import/isolate layers, always manually instantiate the target arrays and use a standard single-pass `for` loop with type casting `if (c is T)` to partition the elements cleanly in $O(N)$ time with minimal allocation overhead.
