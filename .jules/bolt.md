...
## 2025-02-15 - Fast Paths by short-circuiting after sorted arrays
**Learning:** In `lib/features/sleep/data/processing/sleep_pipeline_service.dart`, when iterating through sorted array using `.where(...).toList()`, we'll iterate through all remaining items although the result will be false. This results in an O(N) operation instead of short-circuiting early.
**Action:** When filtering a sorted array up to a certain threshold (like a date constraint), replace `.where(...).toList()` with a standard `for` loop that uses a `break` statement to exit early once the threshold is crossed, lowering the runtime footprint.
