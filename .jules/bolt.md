...
## 2025-02-15 - Removed redundant Iterable allocations in _performBatchImport
**Learning:** In `lib/core/infrastructure/basis_data_manager.dart`, `mappedCompanions` (up to 5000 items per batch) was partitioned into `exerciseBundles` and `otherCompanions` using `.whereType().toList()` and `.where().toList()`. This caused redundant O(N) iterations and unnecessary intermediate array allocations, slowing down batch initialization.
**Action:** Always replace multiple functional `.where()` passes with a single-pass `for` loop to partition collections simultaneously without redundant traversals or list allocations.
