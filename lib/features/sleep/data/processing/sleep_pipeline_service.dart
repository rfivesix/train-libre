...
class SleepPipelineService {
  ...
  static SleepPipelineBackgroundTaskResult _runSleepPipelineBackground(
    ...
    final activeLookbackSessions = params.lookbackSessions
        .map(_toDomainSession)
        .where((existing) {
          final hasIdMatch = mapped.sessions.any(
            (incoming) => incoming.id == existing.id,
          );
          if (hasIdMatch) return false;

          final hasOverlap = mapped.sessions.any(
            (incoming) =>
    ...
    final activeLookbackSessionsRecords = params.lookbackSessions.where((
      dbSession,
    ) {
  ...
  static Map<String, SleepRegularityIndexResult> _buildRegularityByNight({
    ...
    for (final night in targetNights) {
      final history = <DailySleepWakeState>[];
      for (final state in dailyStates) {
        if (!state.day.isAfter(night)) {
          history.add(state);
        } else {
          break; // Optimization: dailyStates is sorted, so we can break early
        }
      }
