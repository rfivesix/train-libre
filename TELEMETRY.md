...
* **Asked Separately From the Mandatory Consent:** The telemetry question is deliberately *not* a checkbox on the initial consent screen. That screen carries only the consent a user must give to use the app at all; bundling an optional choice into the same form makes it far harder to argue the optional one was freely given. Once the obligatory part is settled, a separate bottom sheet asks about anonymous usage statistics, with the accept and the decline as two equally sized buttons in the same row, nothing pre-selected, and the decline exactly one tap away. Telemetry is written to the *off* state before the sheet opens, so a force-quit mid-question can never leave it silently enabled.
* **At Most One Follow-Up, Ever:** If the user declines during onboarding, the same sheet may appear exactly one more time — never a third. The follow-up is gated on **14 calendar days** *and* **5 app launches** since the decline, so it is clearly separated from onboarding and only reaches someone who actually uses the app. It is the last entry in the app's startup-prompt chain (`_runStartupPrompts`), which shows at most one prompt per launch, so it can never stack with the app tour, the release notes, or the review prompt. Once shown it is marked done whatever the answer was — including a dismissal without an answer — and never returns. Installations that predate the prompt are anchored on their first launch after updating and serve the same waiting period rather than being asked on the spot. The state lives in `SharedPreferences`, which the backup includes, so a restored backup carries "already asked" with it. **A decline is never tracked**: without consent nothing may be sent to PostHog, an "opted out" event included.
* **Instant Revocation & Data Deletion:** Users can turn off telemetry at any time in Settings (*Support & Info -> "Share anonymous usage statistics"*, localized into all supported languages). Toggling the switch off immediately disables the PostHog SDK (`Posthog().disable()`). In addition, users can tap **"Delete telemetry data"** directly in Settings (discreet `AppLinkRow` below the telemetry card). Triggering deletion issues `$delete_person` deletion requests to PostHog EU servers for all associated IDs before erasing local device UUIDs, clearing cached counters, and resetting PostHog SDK state (`Posthog().reset()`).
