(() => {
  ...
  const TRANSLATIONS = {
    en: {
      ...
      p_2_l4: "<strong>No Commercial Tracking (Optional Anonymous Telemetry):</strong> Train Libre dispenses with advertising networks, commercial tracking, and behavior profiling. A purely optional, anonymous usage statistics integration (PostHog EU) is disabled by default and captures zero personally identifiable information (PII) or personal workout/meal contents.",
      ...
      p_7_l_telemetry: "<strong>Telemetry Data Rights:</strong> If you have opted in to anonymous telemetry, you may exercise your rights (access, erasure, objection) regarding the processed telemetry data by contacting the controller at feedback@schotte.me. Upon request, all your telemetry events will be deleted from PostHog's servers.",
    ...
    ja: {
      ...
      "p_2_l4": "<strong>商用トラッキングなし（オプションの匿名テレメトリ）：</strong> Train Libreは、広告ネットワーク、商用トラッキング、行動プロファイリングを一切使用しません。完全なオプションである匿名利用統計（PostHog EU）はデフォルトで無効化されており、個人特定情報（PII）や個人のワークアウト・食事内容を送信することはありません。",
