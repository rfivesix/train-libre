(() => {
  ...
  const TRANSLATIONS = {
    en: {
      ...
      p_2_l4: "<strong>No Tracking or Analytics SDKs (Optional Anonymous Telemetry):</strong> Train Libre dispenses with advertising networks, commercial tracking, and behavior profiling. A purely optional, anonymous usage statistics integration (PostHog EU) is disabled by default and captures zero personally identifiable information (PII) or personal workout/meal contents.",
    ...
    ja: {
      ...
      "p_2_l4": "<strong>追跡およびアナリティクスSDK非搭載（オプションの匿名テレメトリ）：</strong> Train Libreは、広告ネットワーク、商用トラッキング、行動プロファイリングを一切使用しません。完全なオプションである匿名利用統計（PostHog EU）はデフォルトで無効化されており、個人特定情報（PII）や個人のワークアウト・食事内容を送信することはありません。",
